...
Nako's long-term flagship client direction. ADR 0031 then sequenced generated
...
- `nako-client` already owns Rust request construction, reqwest execution,
...
- `nako-client-protocol` still has strict serde enums for some additive public
...
Nako will pull shared Rust client-core work forward now, before adding a new
...
- `nako-client-protocol`: permissive Public Client API wire DTOs, public error
  envelope, API constants, route inventory, and public wire vocabularies.
- `nako-client-core`: new permissive, FFI-safe core for request construction,
...
- `nako-client`: Rust async/reqwest adapter that can reuse `nako-client-core`
  while keeping CLI/server tooling ergonomic.
- `nako-client-uniffi`: thin binding crate over `nako-client-core`. It owns
...
4. Rust core interprets status, `x-nako-api-version`, public error envelope,
   and `HealthResponse`.
...
- `nako-client` should gradually become an adapter over the core rather than a
...
- Add a hand-written Kotlin SDK runtime. Rejected as the default because it
  ...
  Rust.
- Bind Android directly to the existing `nako-client`. Rejected because
  `nako-client` currently exposes reqwest, async transport, URL/header/status
