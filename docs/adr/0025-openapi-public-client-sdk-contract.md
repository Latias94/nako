...
Taru now has a permissive public client protocol crate, stable HTTP error
...
The first OpenAPI contract must not make `taru-core`, `taru-server`,
...
Taru will generate the first Public Client API OpenAPI v1 contract from the
AGPL `taru-api` adapter layer while keeping the wire DTO source of truth in
`taru-client-protocol`.

- `taru-client-protocol` owns permissive public wire DTOs, public error codes,
  API version constants, and client-facing enum vocabularies.
- `taru-api` owns OpenAPI aggregation, server-domain-to-protocol mapping, and
  any schema helper code that would otherwise add non-essential dependencies to
  the protocol crate.
- `taru-server` owns HTTP route wiring and behavior tests, but OpenAPI must not
...
Bearer auth, the `x-taru-api-version` header, `ErrorResponse`, pagination, and
...
- The OpenAPI generator gets tests that reject internal crate names, admin-only
  ...
  fields.
- `taru-client-protocol` stays dependency-light by default. If future schema
  derives are needed, they must be optional or live behind the `taru-api`
...
- Generate OpenAPI directly from `taru-server` route handlers. Rejected because
  it couples public client contracts to Axum/server internals and makes
  admin/internal leakage likely.
- Add unconditional OpenAPI derive dependencies to `taru-client-protocol`.
