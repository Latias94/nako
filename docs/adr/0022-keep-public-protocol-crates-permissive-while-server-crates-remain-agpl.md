...
Nako is a network-accessible self-hosted server, so the server implementation
...
At the same time, Nako needs a clean reuse boundary for addon authors, client
applications, SDKs, and integration tools. Those consumers should be able to
depend on Nako wire contracts without inheriting the server implementation
license or depending on Nako's internal domain model.

`nako-addon-protocol` already establishes that pattern by using `Apache-2.0`.
`nako-addon-client` follows the same permissive boundary for the optional Rust
...
Nako server implementation crates remain `AGPL-3.0-or-later` unless a crate is
...
DTOs, manifest types, or response envelopes, Nako should prefer one of these
approaches:
...
Protocol crates should not absorb transport stacks just because Nako's server
...
- Addon authors can reuse Nako protocol crates without inheriting the server
  license.
...
- Protocol crates require more discipline because they cannot casually import
  server internals.
- Optional helper crates such as `nako-addon-client` may carry heavier
...
- Put shared DTOs in `nako-core`: rejected because it would couple external
