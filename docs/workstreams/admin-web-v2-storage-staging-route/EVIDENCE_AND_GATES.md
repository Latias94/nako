# Admin Web V2 Storage Staging Route - Evidence And Gates
...
Use Playwright against `/storage/staging` at desktop and mobile viewports.
Verify nonblank route content, staging rows, no document-level horizontal
...
- `apps/admin-web/src/features/storage/`
...
Storage Staging route and query DTO.
...
Result: passed. Vitest reported 3 test files and 43 tests passing.
...
Result: passed. Vitest reported 4 test files and 45 tests passing. Coverage
...
```bash
playwright-cli open http://127.0.0.1:5173/storage/staging
playwright-cli resize 1440 1000
playwright-cli screenshot --filename=target/admin-web-v2-storage-staging-smoke/desktop.png
playwright-cli resize 390 844
playwright-cli screenshot --filename=target/admin-web-v2-storage-staging-smoke/mobile.png
```
...
- `staging-hls` visible;
- `ffmpeg_input` visible;
- document-level horizontal overflow: false;
- unsafe rendered text scan: empty.

Screenshots:

- `target/admin-web-v2-storage-staging-smoke/desktop.png`
- `target/admin-web-v2-storage-staging-smoke/mobile.png`
...
URL search state, and defers cleanup/delete/repair workflows.
