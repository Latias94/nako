...
The correct first product shape is not a built-in downloader. Nako should first
learn how to discover or accept acquisition candidates, represent them as
Nako-owned staged artifacts, explain their risk, and hand them off to existing
...
DWI-020 is complete. Nako now has durable acquisition intake candidate domain
records and backend-neutral persistence for watch-folder/operator candidates.

DWI-030 is complete. Nako now has an app-service boundary that records/lists
...
DWI-040 is complete. Nako now discovers watch-folder candidates through
...
DWI-050 is complete. Nako now exposes Admin-only, redacted acquisition intake
...
API or `nako-client-protocol` changes.
