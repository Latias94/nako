...
| 2026-05-22 | DWI-060 closeout gate | `cargo nextest run -p taru-db acquisition_intake --no-fail-fast`; `cargo nextest run -p taru-server acquisition_intake --no-fail-fast`; `cargo nextest run -p taru-api admin_contract --no-fail-fast`; `cargo nextest run -p taru-server http::tests::system --no-fail-fast`; `npm run check` from `apps/admin-web`; `cargo fmt --all -- --check`; `python -m json.tool docs/workstreams/downloads-watch-folder-intake/WORKSTREAM.json`; `python -m json.tool docs/workstreams/post-rpd-product-hardening/WORKSTREAM.json`; `git diff --check`; `git diff --name-only -- crates/taru-client-protocol` | Pass. DB acquisition intake contract passed: 1 passed, 123 skipped. Server acquisition intake tests passed: 6 passed, 229 skipped. Admin contract tests passed: 5 passed, 45 skipped. Server system HTTP tests passed: 19 passed, 216 skipped. Admin web typecheck passed. Formatting and both workstream JSON validations passed. Public Client protocol had no changed files. `git diff --check` emitted only repository CRLF conversion warnings for the unrelated `sdk/kotlin` working-tree change. Closeout review found no blocking findings: protocol downloaders, background watch scheduling, Admin UI workflow polish, network access, AI generated artifacts, Addon runtime/distribution, automatic promotion apply, and NFO sidecar mutation shortcuts remain split follow-ons. |
