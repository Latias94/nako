...
Taru has now proven the high-risk foundations that must precede acquisition
breadth:
...
library roots would bypass the very file-write and authority gates Taru just
built.
...
The risk is that Taru could blur four boundaries:
...
- an external downloader's output is not trusted simply because it is present on
  disk;
- a downloader protocol is not part of core Taru until intake, staging,
...
- Taru has an explicit acquisition intake vocabulary for watch-folder and
...
- Admin-only read models expose intake state and blockers without changing the
  Public Client API or `taru-client-protocol`.
...
- Watch-folder candidate discovery through Taru storage/VFS listing and stat
...
| Watch-folder discovery should use VFS/storage boundaries. | High | ADR 0002 and existing `taru-vfs::StorageBackend::list/stat` | If a local filesystem watcher becomes necessary, wrap it behind a storage/intake adapter and keep path safety/redaction centralized. |
...
Keep acquisition as a producer of Taru-owned artifacts, never as a bypass around
promotion/apply:
...
taru-server::app::managed_import / acquisition intake
...
taru-api::admin and taru-server::http::admin
...
```text
Acquisition Intake
  The Taru-owned boundary that receives discovered files, operator candidates,
  or future external downloader outputs before they become Managed Import
  artifacts.

Watch Folder
  A Media Library-scoped storage location that Taru scans for candidates. It is
