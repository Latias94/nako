...
- OpenAPI v1 JSON can be emitted by `nako-api`.
...
- Generate TypeScript/Web/CLI scaffold from OpenAPI:
  `nako_api::sdk::typescript_sdk()` and example emitter.
...
- `crates/nako-api/src/sdk.rs` generates a dependency-free TypeScript SDK
  scaffold from the M32 OpenAPI document.
- `crates/nako-api/examples/emit-typescript-sdk.rs` emits generated TypeScript
  with `cargo run -p nako-api --example emit-typescript-sdk`.
- The scaffold includes OpenAPI-derived TypeScript interfaces, `NakoClient`,
  `NakoApiError`, bearer auth injection, API version inspection, error
  envelope parsing, pagination helpers, playback capability helpers, and core
  route methods.

### SDK-030 SDK Contract Smoke Checks

- `cargo nextest run -p nako-api --no-fail-fast`: passed, 10 tests.
...
- `cargo run -q -p nako-api --example emit-typescript-sdk` smoke output starts
