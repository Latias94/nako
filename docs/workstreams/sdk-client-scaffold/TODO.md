...
- [x] SDK-020 [owner=codex] [deps=SDK-010] [scope=crates/nako-api/src/sdk.rs, crates/nako-api/examples]
  Goal: Add a TypeScript SDK scaffold generator backed by the M32 OpenAPI document.
  Validation: `cargo nextest run -p nako-api --no-fail-fast`, `cargo check -p nako-api --examples`.
  Evidence: `cargo run -p nako-api --example emit-typescript-sdk` emits TypeScript client code.
  Handoff: Keep output dependency-free and do not add Node/Dart packages.

## M33.2 SDK Contract Smoke Checks

- [x] SDK-030 [owner=codex] [deps=SDK-020] [scope=crates/nako-api/src/sdk.rs]
