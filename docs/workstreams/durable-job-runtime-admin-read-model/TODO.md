...
- [x] JRM-020 [owner=codex] [deps=JRM-010] [scope=crates/nako-server/src/app/runtime.rs,crates/nako-server/src/app/jobs.rs,crates/nako-server/src/app/metadata.rs,crates/nako-server/src/app/nfo.rs]
  ...
  --tests`, focused `nako-server` app runtime/jobs/metadata/NFO tests, and
  `git diff --check`.
  Evidence: `crates/nako-server/src/app/job_runtime.rs` plus focused
...
- [x] JRM-030 [owner=codex] [deps=JRM-020] [scope=crates/nako-core,crates/nako-db,crates/nako-api,crates/nako-server/src/http/admin.rs]
  ...
  `cargo nextest run -p nako-api --no-fail-fast`, `cargo check -p nako-db
  --tests`, focused `nako-db` job repository tests, `cargo check -p
