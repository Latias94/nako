...
This workstream tracks M54: deepen Nako's durable job runtime and add the
first Admin API read model for job list/filter views.

## Why This Lane Exists

Nako already has a `RuntimeSupervisor`, durable job rows, startup recovery for
...
- `nako-server::app::job_runtime` centralizes durable job start/succeed/fail
