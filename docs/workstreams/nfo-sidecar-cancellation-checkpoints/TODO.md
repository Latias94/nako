...
- [x] NSCC-010 [owner=planner] [deps=none] [scope=docs/workstreams/nfo-sidecar-cancellation-checkpoints]
  ...
  `nako-nfo` crate owns a server-independent checkpoint contract.

## M1 - NFO Service Checkpoint Contract

- [x] NSCC-020 [owner=codex] [deps=NSCC-010] [scope=crates/nako-nfo/src]
  Goal: Add `nako-nfo` sidecar checkpoint types and no-op-compatible
  import/export service variants without depending on `nako-server`.
  Validation: `cargo check -j 2 -p nako-nfo --tests`; `cargo nextest run -j 2 -p nako-nfo nfo_service --no-fail-fast`.
  ...
  Evidence: New or updated `nako-nfo` tests proving existing no-op paths still
  ...
  `NfoLibraryRunOutcome::Cancelled` without introducing a `nako-server`
  dependency.
  Handoff: Completed with `NSCC-030` and `NSCC-040`.

## M2 - Import Per-Sidecar Checkpoints

- [x] NSCC-030 [owner=codex] [deps=NSCC-020] [scope=crates/nako-nfo/src/import.rs]
  ...
  Validation: `cargo nextest run -j 2 -p nako-nfo import --no-fail-fast`.
...
- [x] NSCC-040 [owner=codex] [deps=NSCC-020] [scope=crates/nako-nfo/src/export.rs]
  ...
  Validation: `cargo nextest run -j 2 -p nako-nfo export --no-fail-fast`.
...
- [x] NSCC-050 [owner=codex] [deps=NSCC-030,NSCC-040] [scope=crates/nako-server/src/app/nfo.rs,crates/nako-server/src/app/tests/nfo.rs,docs/api/HTTP_API.md]
  ...
  Validation: `cargo nextest run -j 2 -p nako-server nfo --no-fail-fast`; `cargo nextest run -j 2 -p nako-server job_cancel --no-fail-fast`.
