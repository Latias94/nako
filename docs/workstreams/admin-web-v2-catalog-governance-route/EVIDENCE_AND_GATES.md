# Admin Web V2 Catalog Governance Route - Evidence And Gates
...
Use Playwright against `/catalog/governance` at desktop and mobile viewports.
Verify nonblank route content, table rows, no document-level horizontal
...
Catalog Governance route and query DTO.
...
Result: passed. Vitest reported 3 test files and 33 tests passing.
...
Result: passed. Vitest reported 4 test files and 35 tests passing. Coverage
...
```bash
playwright-cli open http://127.0.0.1:5173/catalog/governance
playwright-cli resize 1440 1000
playwright-cli screenshot --filename=target/admin-web-v2-catalog-governance-smoke/desktop.png
playwright-cli resize 390 844
playwright-cli screenshot --filename=target/admin-web-v2-catalog-governance-smoke/mobile.png
```

Result: passed. Desktop and mobile checks both found:

- title: `Catalog Governance`;
- `Unmatched OVA Special` visible;
- `low_local_inference_confidence` visible;
- document-level horizontal overflow: false;
- unsafe rendered text scan: empty.

Screenshots:

- `target/admin-web-v2-catalog-governance-smoke/desktop.png`
- `target/admin-web-v2-catalog-governance-smoke/mobile.png`
...
URL search state, and defers detail/repair workflows.
