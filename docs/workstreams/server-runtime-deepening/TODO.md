...
- [x] SRD-020 [owner=codex] [deps=SRD-010] [scope=crates/taru-server/src/app]
  Goal: Add `ServerStartupWorkflow` and move startup side effects out of
  `TaruApp::new_with_store` while preserving behavior.
...
- [x] SRD-030 [owner=codex] [deps=SRD-020] [scope=crates/taru-server/src/app/runtime.rs, crates/taru-server/src/app/jobs.rs, crates/taru-server/src/app/metadata.rs]
  ...
  Validation: `cargo nextest run -p taru-server app::runtime --no-fail-fast`
  passed with 4 tests; `cargo nextest run -p taru-server
