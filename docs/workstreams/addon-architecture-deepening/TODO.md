...
- [x] AAD-020 [owner=codex] [deps=AAD-010] [scope=crates/nako-server/src/app/addons/intake.rs,crates/nako-server/src/app/addons/runtime.rs,crates/nako-server/src/app/addons/side_effect_apply.rs,crates/nako-server/src/app/addons/metadata_write.rs,crates/nako-server/src/app/addons/artwork_write.rs,crates/nako-server/src/app/addons/library_file_write.rs,crates/nako-server/src/app/addons/target.rs,crates/nako-server/src/app/addons/principal.rs,crates/nako-core/src/addon.rs,crates/nako-server/src/http/tests/addons.rs]
  ...
  Validation: `cargo check -p nako-core -p nako-server --tests`; `cargo
...
- [x] AAD-030 [owner=codex] [deps=AAD-020] [scope=crates/nako-core/src/addon.rs,crates/nako-db/migrations,crates/nako-db/migrations/postgres/0001_contract_jobs.sql,crates/nako-db/src/sqlite/addons.rs,crates/nako-db/src/sqlite/codec.rs,crates/nako-db/src/sqlite/migrations.rs,crates/nako-db/src/postgres.rs,crates/nako-db/src/contract_tests.rs,crates/nako-db/src/tests.rs,crates/nako-server/src/app/addons/runtime.rs,crates/nako-server/src/http/tests/addons.rs]
  ...
  `cargo nextest run -p nako-db addon --no-fail-fast`; `cargo nextest run -p
  nako-server addon_side_effect --no-fail-fast`; PostgreSQL opt-in contract
  ...
  Progress: Added `AddonSideEffectRequestFingerprint` to `nako-core` and
  ...
  fingerprint into the clean base schemas because Nako has no deployed users.
  PostgreSQL schema parity was updated in
  `migrations/postgres/0001_contract_jobs.sql`; opt-in PostgreSQL contracts
  were skipped because `NAKO_TEST_POSTGRES_URL` was not set.
  Validation: `cargo check -p nako-core -p nako-db -p nako-server --tests`;
  `cargo nextest run -p nako-db addon --no-fail-fast`; `cargo nextest run -p
  nako-server addon_side_effect --no-fail-fast`; `cargo fmt --all -- --check`;
  `git diff --check`.
...
- [x] AAD-040 [owner=codex] [deps=AAD-030] [scope=crates/nako-addon-protocol,crates/nako-api/src/extension.rs,crates/nako-reference-addon,crates/nako-server/src/app/addons/metadata_write.rs,crates/nako-server/src/app/addons/artwork_write.rs,crates/nako-server/src/app/addons/library_file_write.rs,crates/nako-server/src/http/tests/addons.rs,docs/api/HTTP_API.md,docs/guides/ADDON_AUTHOR_GUIDE.md]
  ...
  Write command.
  Validation: `cargo check -p nako-addon-protocol -p nako-api -p
  nako-reference-addon -p nako-server --tests`; `cargo nextest run -p
  nako-addon-protocol --no-fail-fast`; `cargo nextest run -p nako-server
  addon_side_effect --no-fail-fast`; `git diff --check`.
  Review: Preserve the permissive protocol boundary. Do not move server-only
  records into `nako-addon-protocol`. The Interface should be Addon-author
  useful, not a mirror of persistence records.
  Handoff: Continue with AAD-050 Addon Manifest depth.
  Progress: Added Addon-facing payload DTOs to `nako-addon-protocol`:
  ...
  keeping Nako-only validation, merge, candidate, storage, and report behavior
  inside `nako-server`. Added protocol wire-shape tests and reference Addon
...
- [x] AAD-050 [owner=codex] [deps=AAD-040] [scope=crates/nako-addon-protocol,crates/nako-reference-addon,crates/nako-server/src/app/addons.rs,crates/nako-server/src/http/tests/addons.rs,docs/guides/ADDON_AUTHOR_GUIDE.md,docs/api/HTTP_API.md,docs/workstreams/addon-architecture-deepening]
  ...
  Validation: `cargo check -p nako-addon-protocol -p nako-reference-addon -p
  nako-server --tests`; `cargo nextest run -p nako-addon-protocol
  --no-fail-fast`; `cargo nextest run -p nako-server register_addon
...
- [x] AAD-060 [owner=codex] [deps=AAD-040] [scope=crates/nako-server/src/app/addons/library_file_write.rs,crates/nako-server/src/app/nfo.rs,crates/nako-nfo,crates/nako-vfs,crates/nako-core/src/addon.rs,crates/nako-server/src/http/tests/addons.rs,docs/workstreams/addon-library-file-write-policy,docs/api/HTTP_API.md]
  Goal: Deepen Library File Write into a Nako-owned runtime Module that owns
  ...
  Validation: `cargo check -p nako-core -p nako-nfo -p nako-vfs -p
  nako-server --tests`; `cargo nextest run -p nako-server
  ...
  Validation: `cargo check -p nako-core -p nako-nfo -p nako-vfs -p
  nako-server --tests`; `cargo nextest run -p nako-server
  library_file_write --no-fail-fast`; `cargo nextest run -p nako-nfo
...
- [x] AAD-070 [owner=codex] [deps=AAD-010,AAD-020] [scope=crates/nako-api/src/extension.rs,crates/nako-server/src/http/addons.rs,crates/nako-server/src/app/addons.rs,crates/nako-server/src/http/tests/addons.rs,docs/api/HTTP_API.md,docs/workstreams/admin-web-console]
  ...
  nextest run -p nako-server addons --no-fail-fast`; `cargo nextest run -p
  nako-api --no-fail-fast`; `cargo fmt --all -- --check`; `git diff --check`.
...
- [x] AAD-080 [owner=codex] [deps=AAD-050] [scope=crates/nako-addon-protocol,crates/nako-addon-client,Cargo.toml,crates/nako-reference-addon,docs/adr/0022-keep-public-protocol-crates-permissive-while-server-crates-remain-agpl.md,docs/workstreams/addon-architecture-deepening]
  Goal: Audit whether `nako-addon-protocol` should remain both wire contract
  ...
  -p nako-server reference_addon_registers_queries_and_handles_resource_call
  --no-fail-fast`; `cargo fmt --all -- --check`; `git diff --check`.
  Review: `nako-addon-protocol` now owns wire contracts and validation without
  `reqwest` or `async-trait`; `nako-addon-client` owns `AddonTransport`,
  ...
  dependencies; HTTP caller tests moved to `nako-addon-client`.
  Handoff: Continue with AAD-090 Addon persistence parity.

- [x] AAD-090 [owner=codex] [deps=AAD-030,AAD-070] [scope=crates/nako-db/migrations,crates/nako-db/migrations/postgres,crates/nako-db/src/sqlite/addons.rs,crates/nako-db/src/postgres.rs,crates/nako-db/src/contract_tests.rs,docs/workstreams/postgresql-production-readiness]
  ...
  addon --no-fail-fast`; PostgreSQL opt-in `cargo nextest run -p nako-db
  addon --run-ignored ignored-only --no-fail-fast` when
  `NAKO_TEST_POSTGRES_URL` is available; `git diff --check`.
  Review: Any schema change must have backend-neutral contract evidence. Nako
...
- [x] AAD-100 [owner=planner] [deps=AAD-090] [scope=docs/workstreams/addon-architecture-deepening,docs/GOALS.md,docs/ROADMAP.md,docs/workstreams/README.md]
  ...
  --no-fail-fast`; `cargo nextest run -p nako-db addon --no-fail-fast`;
  `cargo nextest run -p nako-server addons --no-fail-fast`; `cargo nextest run
  -p nako-server addon_side_effect --no-fail-fast`; `cargo nextest run -p
  nako-server library_file_write --no-fail-fast`; `cargo nextest run -p
  nako-api --no-fail-fast`; `cargo nextest run --workspace --no-fail-fast`;
