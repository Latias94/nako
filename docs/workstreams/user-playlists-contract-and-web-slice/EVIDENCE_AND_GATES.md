...
| 2026-05-29 | UPCW-030 | Implemented principal-scoped User Playlist records/repository trait, SQLite/PostgreSQL baseline schema and adapters, NakoDatabase facade forwarding, `UserPlaylistAppService`, and repository/app tests for idempotent membership, ordering/reorder, stale version conflicts, invalid names, media item existence, owner scope, and delete cascade. Validation: `cargo nextest run -p nako-db playlist --no-fail-fast`; `cargo nextest run -p nako-server user_playlist --no-fail-fast`; `NAKO_TEST_POSTGRES_URL=<ephemeral Docker PostgreSQL> cargo nextest run -p nako-db postgres_playback_runtime_contract_user_playlist_membership_is_principal_scoped_ordered_and_idempotent --run-ignored ignored-only --no-fail-fast`; `cargo fmt --all -- --check`; `python -m json.tool docs/workstreams/user-playlists-contract-and-web-slice/WORKSTREAM.json`; `git diff --check -- docs/workstreams/user-playlists-contract-and-web-slice crates/nako-core crates/nako-db crates/nako-server/src/app`. | Passed. |
| 2026-05-29 | UPCW-040 | Exposed current-user User Playlist routes through Public Client HTTP, added Public Client DTO mapping for access-filtered playlist counts and item responses, enforced effective Library Access before item mutations and while listing items, added Rust client methods, and tightened SDK inventory coverage. Validation: `cargo nextest run -p nako-api playlist --no-fail-fast`; `cargo nextest run -p nako-server user_playlist --no-fail-fast`; `cargo nextest run -p nako-client user_playlist --no-fail-fast`; `cargo nextest run -p nako-client sdk_inventory --no-fail-fast`; `npm run check --prefix sdk/typescript`; `cargo fmt --all -- --check`; `python -m json.tool docs/workstreams/user-playlists-contract-and-web-slice/WORKSTREAM.json`; `git diff --check -- docs/workstreams/user-playlists-contract-and-web-slice crates/nako-api crates/nako-server/src/http crates/nako-client sdk/typescript`. | Passed. |
| 2026-05-29 | UPCW-050 | Restored the first `web/` playlist UI through Public Client live data and fixture fallback. Added data-source mappings for `/users/me/playlists`, TanStack Query hooks, `/media/my-list` route search state, route/data-source/state tests, and desktop/mobile browser smoke screenshots. Validation: `npm --prefix web run test -- src/test/data-source-contracts.test.ts`; `npm --prefix web run test -- src/test/route-contracts.test.tsx src/test/route-state-contracts.test.tsx`; `npm --prefix web run test`; `npm --prefix web run check`; `npm --prefix web run build:budget`; browser smoke at `http://127.0.0.1:5174/media/my-list` and mobile `390x844` viewport with screenshots under `web/test-results/`. | Passed. |
| 2026-05-29 | UPCW-060 | Closed the lane after confirming backend/API/SDK/web evidence covers the target state. Recorded closeout, follow-ons, residual risks, and closed workstream metadata. Validation: `python -m json.tool docs/workstreams/user-playlists-contract-and-web-slice/WORKSTREAM.json`; `git diff --check`. | Passed. |
