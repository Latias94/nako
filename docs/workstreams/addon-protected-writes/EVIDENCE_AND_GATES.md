...
that a valid `metadata_write` side effect applies through Nako-owned metadata
...
write path. The gate must prove Library File Write behavior remains Nako-owned,
...
- Addon Side Effect intake anchors:
  - `crates/nako-core/src/addon.rs` defines `AddonPermission`,
    `AddonSideEffectTargetKind`, `AddonSideEffectValidationStatus`,
    `NewAddonSideEffect`, `AddonSideEffectRecord`, and `AddonPrincipal`.
  - `crates/nako-db/migrations/0022_addon_side_effects.sql` stores actor,
    ...
    creation time.
  - `crates/nako-db/src/addons.rs` persists intake idempotently by
    `(addon_id, idempotency_key)`.
  - `crates/nako-api/src/extension.rs` exposes generic JSON payload/provenance
    on request, but response summaries exclude raw payload/provenance.
  - `crates/nako-server/src/app/addons.rs` authenticates the Addon Token,
...
- Canonical Metadata seam anchors:
  - `crates/nako-core/src/media/item.rs` owns `CanonicalMetadata`.
  - `crates/nako-core/src/media/merge.rs` owns `MetadataMergePolicy`,
    `MetadataMergeMode`, source-aware lock handling, and populated-field
    discovery.
  - `crates/nako-metadata/src/strategy.rs` uses `MetadataMergePolicy`,
    `MetadataRefreshPort`, `MetadataAttemptPort`, and `hydrate_item_catalog`
    for provider refresh.
  - `crates/nako-nfo/src/import.rs` uses
  ...
  - `crates/nako-db/src/metadata.rs` exposes
...
- Metadata gap:
  - Existing metadata commit is provider-shaped because it requires
    ...
    Nako-owned commit path that does not fabricate provider subjects or raw
    provider responses.
  - `crates/nako-core/src/media/metadata.rs::MetadataSource` has `Local`,
    `Nfo`, `Provider`, and `User`, but no Addon variant.
  - `crates/nako-db/src/codec.rs::metadata_source_from_parts` maps unknown
...
- Catalog/search seam anchors:
  - `crates/nako-catalog/src/lib.rs::hydrate_item_catalog` loads the current
    item, builds graph replacement from Canonical Metadata, builds search
    projection, and commits both through `CatalogRepository`.
  - `crates/nako-core/src/repository/catalog.rs::commit_item_projection`
    declares the graph-plus-search commit boundary.
  - `crates/nako-db/src/catalog.rs::commit_item_projection` commits graph
    replacement and search projection in one SQLite transaction.
- Event/audit gap:
  - `crates/nako-core/src/event.rs::DomainEventKind` has
...
- Artwork seam anchors:
  - `crates/nako-core/src/media/catalog.rs::ImageAsset` stores artwork-like
    asset records with source URI, optional cache URI, dimensions, language,
    selected state, hash, and etag.
  - `crates/nako-core/src/media/artwork.rs::ArtworkTask` and
    `crates/nako-db/src/artwork.rs` provide task persistence for fetch, resize,
    preview, and cleanup.
- Artwork gap:
...
- First apply target decision:
  ...
  - Required APW-030 preconditions: explicit side-effect apply outcome state,
    ...
    merge through `MetadataMergePolicy`, Nako-owned persistence, catalog/search
    hydration, redacted response, and idempotent replay after apply.
- ADR impact:
  - No ADR amendment is required if APW-030 preserves ADR 0020 and Nako-owned
...
- Implemented explicit Addon Side Effect apply outcome state:
  - `crates/nako-core/src/addon.rs` adds `AddonSideEffectApplyStatus` and
  ...
  - `crates/nako-db/migrations/0023_addon_side_effect_apply_outcome.sql` adds
    ...
    and `applied_at`.
  - `crates/nako-db/src/addons.rs` persists and reloads apply outcomes through
    `set_addon_side_effect_apply_outcome`.
- Implemented first-class Addon metadata attribution:
  - `crates/nako-core/src/media/metadata.rs` adds
    `MetadataSource::Addon(AddonId)`.
  - `crates/nako-db/src/codec.rs` stores it as `source = addon` with
    `source_key = <addon_id>`, avoiding fake provider attribution.
  - `crates/nako-client-protocol/src/catalog.rs` and
    `crates/nako-api/src/public_client.rs` expose a stable public Addon source
    DTO for catalog records that include Addon-sourced genres/tags.
- Implemented the first concrete `metadata_write` apply path:
  - `crates/nako-server/src/app/addons.rs` still performs Addon Token
...
- Tests prove:
  - `crates/nako-db/src/tests.rs` round-trips Addon metadata source attribution
    and records Addon Side Effect apply outcomes.
  - `crates/nako-server/src/http/tests/addons.rs` applies an authorized
...
- Validation run before documentation closeout:
  - `cargo check -p nako-core -p nako-db -p nako-api -p nako-server -p nako-metadata -p nako-catalog --tests`
    exited 0.
  - `cargo nextest run -p nako-db addon --no-fail-fast` exited 0 with 5 tests
    passed.
  - `cargo nextest run -p nako-db sqlite_store_round_trips_metadata_policy_records --no-fail-fast`
    exited 0 with 1 test passed.
  - `cargo nextest run -p nako-server addon_side_effect --no-fail-fast`
    exited 0 with 3 tests passed.
  - `cargo nextest run -p nako-catalog --no-fail-fast` exited 0 with 3 tests
    passed.
  - `cargo fmt --all -- --check` exited 0.
  - `git diff --check` exited 0.
  - `Get-Content docs/workstreams/addon-protected-writes/WORKSTREAM.json | ConvertFrom-Json | Out-Null`
...
- `cargo nextest run -p nako-server addon_side_effect --no-fail-fast` exited 0
  with 3 tests passed.
- `cargo check -p nako-core -p nako-db -p nako-api -p nako-server
  -p nako-metadata -p nako-catalog --tests` exited 0.
...
- Review result:
  - Workstream compliance had no blocking findings after APW-030 because the
    lane proved a Nako-owned protected-write apply model for Canonical
    Metadata.
...
- Fix applied before closeout:
  - `crates/nako-core/src/repository/catalog.rs` adds a narrow search
    projection update seam.
  - `crates/nako-db/src/catalog.rs` implements search-only projection update.
  - `crates/nako-catalog/src/lib.rs` adds search-only refresh and selected
    genre/tag hydration helpers.
  - `crates/nako-server/src/app/addons.rs` now refreshes search for scalar
    metadata patches and only replaces touched genre/tag label sets with
    `MetadataSource::Addon(addon_id)`.
  - `crates/nako-server/src/http/tests/addons.rs` proves scalar-only and
...
- Follow-on split:
  - `docs/workstreams/addon-managed-artwork-artifacts/` owns `artwork_write`,
    Artwork Candidate, Managed Artwork, Nako-Managed Artifact storage, fetch
...
- Existing APW runtime behavior has only one concrete Addon write path:
  `metadata_write` in `crates/nako-server/src/app/addons.rs`.
- No current Addon runtime path writes NFO files, subtitle files, Managed
  ...
  field locks.
- Follow-on documentation now points AMAA and ALFW at Nako-owned commit/service
