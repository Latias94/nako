...
Taru can now accept and audit an Addon Side Effect, but the accepted record is
not yet an applied domain change. If concrete handlers grow one by one without
a shared apply model, Taru risks reintroducing the shortcuts the intake lane was
created to prevent:
...
- Canonical Metadata writes pass through Taru's metadata merge and catalog
  consistency seams.
- Managed Artwork, subtitle, NFO, and other Library File Write behavior uses
  Taru storage/VFS and sidecar write policy rather than addon-provided paths.
...
- Define how Managed Artwork and Taru-Managed Artifact outputs enter Taru from
  an Addon Sidecar.
...
| Library File Write behavior must reuse Taru storage/VFS policy. | High | `CONTEXT.md` and NFO write lanes reject direct addon path writes. | Protected file writes remain deferred until the policy seam is explicit. |
...
Keep protected writes as a three-stage Taru workflow:
...
2. Effect-specific validation normalizes the payload into a Taru domain command
   such as Canonical Metadata update, Managed Artwork candidate/import, subtitle
   import, NFO Export, or Library File Write.
3. Domain apply calls the owning Taru service/repository boundary and records a
...
catalog services over embedding write logic inside `taru-server/src/http`.
