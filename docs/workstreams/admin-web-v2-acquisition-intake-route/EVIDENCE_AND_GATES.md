# Admin Web V2 Acquisition Intake Route Evidence And Gates
...
Run the dev server and smoke `/acquisition/intake` at:
...
- unsafe rendered text scan is empty for raw source refs, source URIs, local
  paths, credentials, raw locator bodies, and tokens.
...
Acquisition Intake route and existing generated query/response types.
...
npx --no-install playwright-cli -s admin-aw-v2-remaining-routes open http://127.0.0.1:5180/acquisition/intake
npx --no-install playwright-cli -s admin-aw-v2-remaining-routes resize 1440 1000
npx --no-install playwright-cli -s admin-aw-v2-remaining-routes screenshot --filename=../../target/admin-web-v2-acquisition-intake-smoke/desktop.png
npx --no-install playwright-cli -s admin-aw-v2-remaining-routes resize 390 844
npx --no-install playwright-cli -s admin-aw-v2-remaining-routes screenshot --filename=../../target/admin-web-v2-acquisition-intake-smoke/mobile.png
```

Result: passed. Desktop and mobile checks found:

- title: `Acquisition Intake`;
- `candidate-ready` visible;
- document-level horizontal overflow: false;
- unsafe rendered text scan: empty for raw tokens, raw source refs, source
  URIs, raw locators, Windows paths, and `/Users/` paths;
- browser console errors: 0.

Screenshots:

- `target/admin-web-v2-acquisition-intake-smoke/desktop.png`
- `target/admin-web-v2-acquisition-intake-smoke/mobile.png`
