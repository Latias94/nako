...
The lane is closed. Future regressions should start from the focused playback
and HLS gates:
...
| 2026-05-29 | PALD-040 | Closeout verification and follow-on split | Passed | Fresh gates: `cargo nextest run -p nako-playback audio --no-fail-fast` (4 passed, 19 skipped); `cargo nextest run -p nako-server hls --no-fail-fast` (56 passed, 422 skipped); `cargo nextest run -p nako-server playback --no-fail-fast` (135 passed, 343 skipped); `cargo nextest run -p nako-api --no-fail-fast` (69 passed, 0 skipped). Review: no blocking findings. Architecture docs already linked this lane; closeout updated status wording and deferred persisted settings, subtitle policy, codec-aware audio, downmix/normalization, UI controls, LL-HLS, DASH, DRM, and offline sync as follow-ons. Final hygiene: `cargo fmt --all -- --check`; `python3 -m json.tool docs/workstreams/playback-audio-language-default-policy/WORKSTREAM.json`; `git diff --check`. |
...
- `docs/workstreams/playback-audio-language-default-policy/CLOSEOUT.md`
