...
| 2026-05-26 | MWF-040 browse/search/detail | `cd apps/admin-web && npm run test -- mediaSurface.test.tsx mediaDataSource.test.ts`; `cd apps/admin-web && npm run test -- App.test.tsx mediaSurface.test.tsx mediaDataSource.test.ts`; `cd apps/admin-web && npm run check`; `cd apps/admin-web && npm run test`; `cd apps/admin-web && npm run build`; `rg -n "admin/v1\|AdminApi\|adminApi\|source_locator\|local path\|ffmpeg" apps/admin-web/src/surfaces/media`; Playwright fixture smoke for `/media/libraries`, `/media/libraries/:libraryId`, `/media/items/:itemId`, and `/media/search?q=rain` | DONE. Focused tests passed 103/103, package tests passed 153/153, TypeScript check and Vite build passed, boundary grep returned no matches, and browser smoke confirmed route navigation plus URL-owned search state. Build retained the existing Vite large chunk advisory. |
| 2026-05-26 | MWF-050 source selection/playback shell | `cd apps/admin-web && npm run test -- mediaSurface.test.tsx mediaDataSource.test.ts`; `cd apps/admin-web && npm run test -- App.test.tsx mediaSurface.test.tsx mediaDataSource.test.ts`; `cd apps/admin-web && npm run check`; `cd apps/admin-web && npm run test`; `cd apps/admin-web && npm run build`; `rg -n "admin/v1\|AdminApi\|adminApi\|source_locator\|local path\|ffmpeg" apps/admin-web/src/surfaces/media`; Playwright fixture smoke from `/media` to `/media/items/:itemId` and `/media/watch/:itemId?source_id=...` | DONE_WITH_CONCERNS. Focused Media tests passed 13/13, focused App/Media tests passed 107/107, package tests passed 157/157, TypeScript check and Vite build passed, boundary grep returned no matches, and browser smoke confirmed source URL state, playback decision updates, safe watch shell, and watched-state writes. Build retained the existing Vite large chunk advisory; real browser media playback remains gated on MWF-GAP-004. |
| 2026-05-26 | MWF-060 closeout | `python -m json.tool docs/workstreams/media-web-client-foundation/WORKSTREAM.json`; `git diff --check`; `cd apps/admin-web && npm run check`; `cd apps/admin-web && npm run test`; `cd apps/admin-web && npm run build`; `rg -n "admin/v1\|AdminApi\|adminApi\|source_locator\|local path\|ffmpeg" apps/admin-web/src/surfaces/media`; Playwright desktop/mobile screenshots for `/media/watch/:itemId?source_id=...`; `CLOSEOUT.md`; `FOLLOW_ON_SPLIT.md` | DONE. Lane closed with no blocking review findings. Package tests passed 157/157, TypeScript check and Vite build passed, JSON and whitespace checks passed, boundary grep returned no matches, and follow-ons were split. Rust/Public API gates were not rerun in MWF-060 because the closeout changed only frontend/docs and no Rust/API/SDK files changed after the MWF-020 contract gate. |
