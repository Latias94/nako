...
The Generated SDK Runtime Ownership lane is complete. Taru has pulled the
...
- Frozen ownership matrix for generated SDK, Android app, Rust client core,
  UniFFI binding, `taru-client`, and follow-on SDK/runtime surfaces.
- ADR 0032:
  `docs/adr/0032-shared-rust-client-core-app-supplied-transport.md`.
- `crates/taru-client-core`:
...
- `crates/taru-client-uniffi`:
  - thin UniFFI binding surface over `taru-client-core`;
  - no runtime policy in the binding crate.
- `crates/taru-uniffi-bindgen`:
...
- Android connection consumption:
  ...
  - `TaruConnectionClient` uses the Rust core for connection probe
...
test matrices, `taru-client` reuse, and build packaging.
...
- The core is no-socket and FFI-safe; it does not expose `reqwest`, async
  ...
  UniFFI.
- The UniFFI crate delegates to `taru-client-core` and does not become a policy
  owner.
...
2. **Rust client-core adapter reuse**
   - Refactor `taru-client` to reuse `taru-client-core` so Rust
...
- The connection tracer intentionally avoids strict-enum browse/playback DTO
  ...
  behind UniFFI.
- `taru-client` and `taru-client-core` can drift until adapter reuse is
  implemented.
...
- `crates/taru-uniffi-bindgen/src/main.rs`
