...
Nako already has an external automation foundation and durable Automation
...
untrusted generated proposal into a reviewable Nako artifact that can only
...
- Nako has a stable Generated Artifact proposal/readiness vocabulary for
...
- Accepted metadata/title proposals route through existing Nako authority
...
| --- | --- | --- | --- |
| AI should enter through external automation first, not a local runtime. | High | ADR 0004 and `nako-automation` foundation | If local model execution becomes urgent, split a runtime lane after generated artifact semantics are stable. |
...
Keep AI as an external proposal source and Nako as the authority boundary:
...
`nako-api::admin`, not by the Public Client protocol. The DTO mirrors only the
...
kept in sync with the Rust generator, while `nako-client-protocol` remains
