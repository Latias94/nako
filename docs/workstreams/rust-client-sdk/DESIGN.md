...
`taru-client-protocol` DTOs instead of generating duplicate Rust wire types
from OpenAPI.
...
- `taru-client-protocol` is already `Apache-2.0` and dependency-light.
...
- The workspace default license is AGPL, so `taru-client` must override its
  crate license explicitly.
- `taru-api` owns the OpenAPI route inventory, but `taru-client` should not
  depend on `taru-api` because that would pull server/domain adapter crates
  across the public SDK boundary.

## Target State

- `crates/taru-client` exists as an `Apache-2.0` Rust SDK crate.
- The crate depends on `taru-client-protocol` for wire DTOs and does not depend
  on server/internal crates.
- The SDK exposes `TaruClient`, client configuration, pagination and playback
...
- Add `crates/taru-client` with explicit permissive license metadata.
...
- Any dependency from `taru-client` to server/internal crates.

## Architecture Direction

`taru-client` is the Rust SDK runtime crate. It should own HTTP mechanics:
...
It should not own wire DTOs. Those remain in `taru-client-protocol`, preserving
...
`taru-client` depend on `taru-api`. The first M35 slice can keep a local
`PUBLIC_CLIENT_PATHS` inventory in `taru-client` and verify SDK coverage against
...
- `crates/taru-client` compiles and tests pass;
