...
| 2026-05-29 | WAAI-040 | Decided that watch-folder discovery mutation controls must split to a future guarded mutation lane. The generated Admin route is present, but the UI needs explicit permission, confirmation, idempotency, redacted result, loading/failure, and no-promotion/no-library-write guarantees before implementation. Validation: `python -m json.tool docs/workstreams/web-admin-acquisition-intake/WORKSTREAM.json`; `git diff --check -- docs/workstreams/web-admin-acquisition-intake`. | Passed. |
| 2026-05-29 | WAAI-050 | Closed the lane after final review found no blocking workstream or code-quality findings. Validation: `npm --prefix web run test` passed with 72 tests; `npm --prefix web run check` passed; `npm --prefix web run build:budget` passed with initial JS 445.70 KiB raw / 137.48 KiB gzip and Admin route JS 225.96 KiB raw / 49.06 KiB gzip under budget. Browser smoke passed at `1280x720` and `390x844`; screenshots saved at `target/waai-050-desktop.png` and `target/waai-050-mobile.png`. Desktop smoke confirmed heading, candidate content, redaction text, no raw path, no prompt text, and no horizontal overflow. Mobile smoke confirmed heading, candidate content, library filter state, and no horizontal overflow. | Passed. |
