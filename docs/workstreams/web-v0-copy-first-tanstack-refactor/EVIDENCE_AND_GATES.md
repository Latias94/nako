...
| 2026-05-28 | WVTR-050 partial | `npm --prefix web install @nako/sdk@file:../sdk/typescript`; `npm --prefix web run check`; `npm --prefix web run test`; `npm --prefix sdk/typescript run check`; `npm --prefix web run build`; static `/media` smoke through `playwright-cli` with console 0 errors and 0 warnings. | Public Client media data source added with fixture fallback. Admin API surface reattachment remains open. |
...
| Live Nako data | Partial | Media home/search/detail now enter through a Public Client data source with fixture fallback. Admin surfaces still use copied v0 static state; WVTR-050 remains open for Admin API reattachment. |
