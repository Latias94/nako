...
- `cargo nextest run -p nako-db nfo --no-fail-fast`
- `git diff --check`
...
- `cargo nextest run -p nako-db scan --no-fail-fast`
- `git diff --check`

CAD-040:

- `cargo check -p nako-server --tests`
- `cargo nextest run -p nako-server app --no-fail-fast`
...
- 2026-05-18: CAD-020 implemented NFO import atomic commit.
  ...
  - `cargo check -p nako-catalog --tests`: passed; added because CAD-020 now
...
- 2026-05-18: CAD-030 implemented Library Scan Source Commit.
  ...
  - `cargo check -p nako-catalog --tests`: passed as a touched dependency
    check from CAD-020.
  - `cargo check -p nako-nfo --tests`: passed as a touched dependency check
...
- 2026-05-18: CAD-050 replaced weak playback/transcode request identity with
  ...
  - `cargo check -p nako-api --tests`: passed; added because DTO sample tests
...
- 2026-05-18: CAD-070 aligned Addon Sidecar protected-write follow-ons with the
  ...
  - Behavior proven: no current Addon runtime path writes NFO files, subtitle
    ...
    artwork multi-row persistence to use or introduce a Nako-owned
...
- 2026-05-18: CAD-080 deletion sweep removed stale test anchors and hardened
  ...
  - Production write-path scan:
    `rg -n "record_scanned_media_source|upsert_source_state\\(|upsert_local_inference_evidence\\(|commit_library_scan_source\\(" crates/nako-library crates/nako-server crates/nako-db/src/scan.rs crates/nako-core/src/repository/scan.rs`.
  - NFO write-path scan:
    `rg -n "upsert_field_lock\\(|confirm_hierarchy\\(|hydrate_item_catalog\\(|commit_nfo_import\\(" crates/nako-nfo/src/import.rs crates/nako-nfo/src/lib.rs crates/nako-server/src/app/nfo.rs crates/nako-db/src/metadata.rs crates/nako-core/src/repository/metadata.rs`.
  - `cargo check -p nako-transcode --tests`: passed.
  - `cargo check -p nako-db --tests`: passed.
  ...
  - Behavior proven: obsolete production request-key constants are no longer
    ...
    `nako-nfo` import or `nako-library` source indexing; fake smoke-probe
