...
- [x] APDV-020 [owner=codex] [deps=APDV-010] [scope=apps/android/scripts/Smoke-Emulator.ps1,apps/android/SMOKE_FIXTURES.md,docs/workstreams/android-playback-depth-validation]
  ...
  Handoff: DONE on 2026-05-19. Player smoke now waits for `Ended` and requires
  `00:02`, proving the short Direct Play fixture advanced beyond the seeded
  `0:01` server resume point.

## M2 - Server Progress Readback After Exit

- [x] APDV-030 [owner=codex] [deps=APDV-020] [scope=apps/android/scripts/Smoke-Emulator.ps1,apps/android/README.md,docs/workstreams/android-playback-depth-validation]
  ...
  Handoff: DONE on 2026-05-19. Smoke now writes
  `profile-with-media-server-readback.txt` after leaving the player and asserts
  `watched=true` plus zero Continue Watching rows.

## M3 - Closeout

- [x] APDV-040 [owner=planner] [deps=APDV-030] [scope=docs/workstreams/android-playback-depth-validation]
  ...
  Handoff: DONE on 2026-05-19. Lane is closed. HLS/remux/session cancellation,
  longer watched-threshold media, and playback quality checks remain separate
  lanes.
