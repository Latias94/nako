...
- [x] ARD-020 [owner=codex] [deps=ARD-010] [scope=crates/nako-addon-protocol,crates/nako-api/src/extension.rs,crates/nako-server/src/app,crates/nako-server/src/http/addons.rs]
  ...
  Validation: focused `nako-addon-protocol` tests; focused Admin DTO/server
  tests for redacted install-guide previews; `cargo fmt --all -- --check`;
  `git diff --check`; `git diff --name-only -- crates/nako-client-protocol`.
  Evidence: `cargo nextest run -p nako-addon-protocol --no-fail-fast`; `cargo
  nextest run -p nako-server admin_addon_install_guide_preview
  --no-fail-fast`; `cargo nextest run -p nako-server addons --no-fail-fast`;
  `cargo nextest run -p nako-api --no-fail-fast`; `cargo fmt --all -- --check`;
  `git diff --check`; `git diff --name-only -- crates/nako-client-protocol`.
...
- [x] ARD-030 [owner=codex] [deps=ARD-020] [scope=crates/nako-addon-client,crates/nako-server/src/app,crates/nako-api/src/extension.rs,crates/nako-server/src/http/addons.rs,apps/admin-web/src/adminApi]
  ...
  nako-server addons --no-fail-fast`; `cargo nextest run -p nako-api
  ...
  Evidence: `cargo nextest run -p nako-server admin_addon_runtime_readiness
  --no-fail-fast`; `cargo nextest run -p nako-server addons --no-fail-fast`;
  `cargo nextest run -p nako-api admin_contract --no-fail-fast`; `npm run
...
- [x] ARD-040 [owner=codex] [deps=ARD-030] [scope=crates/nako-core,crates/nako-db,crates/nako-server/src/app,crates/nako-server/src/http/admin.rs]
  Goal: Turn manifest-declared Addon Tasks and Event Subscriptions into
  Nako-owned routing plans: executable only where existing job/outbox/addon
  ...
  Evidence: `cargo check -p nako-api -p nako-server --tests`; `cargo nextest
  run -p nako-db addon --no-fail-fast`; `cargo nextest run -p nako-server
  admin_addon_routing_plans --no-fail-fast`; `cargo nextest run -p nako-server
  addons --no-fail-fast`; `cargo nextest run -p nako-api admin_contract
...
- [x] ARD-050 [owner=codex] [deps=ARD-040] [scope=crates/nako-api/src/extension.rs,crates/nako-server/src/app,crates/nako-server/src/http/addons.rs,crates/nako-server/src/http/tests]
  ...
  `cargo check -p nako-api -p nako-server --tests`; `cargo fmt --all
  -- --check`; `git diff --check`; `git diff --name-only --
  crates/nako-client-protocol`.
  Review: check acceptance/audit routing, redaction, and no direct canonical
  or library mutation.
  Evidence: `cargo nextest run -p nako-server
  addon_generated_artifact_handoff --no-fail-fast`; `cargo nextest run -p
  nako-server addon_acquisition_candidate_handoff --no-fail-fast`; `cargo
  nextest run -p nako-server addon_handoff --no-fail-fast`; `cargo nextest run
  -p nako-api admin_contract --no-fail-fast`; `cargo fmt --all -- --check`;
  `git diff --check`; `git diff --name-only -- crates/nako-client-protocol`.
...
- [x] ARD-060 [owner=planner] [deps=ARD-050] [scope=docs/workstreams/addon-runtime-and-distribution,docs/workstreams/post-rpd-product-hardening,docs/workstreams/README.md]
  ...
  and `nako-client-protocol` changes were split rather than hidden in this
  lane.
