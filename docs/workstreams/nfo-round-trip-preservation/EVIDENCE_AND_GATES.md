...
- ADR 0008 treats NFO as a local metadata boundary behind `taru-nfo`.
...
- 2026-05-17: Added `NfoPreservedRender`, `NfoPreservationReport`,
  `NfoFieldConflict`, and `NfoFieldConflictReason` to `taru-nfo`.
...
- 2026-05-17: Focused validation passed:
  - `cargo fmt --all -- --check`.
  - `cargo check -p taru-nfo --tests`.
  - `cargo nextest run -p taru-nfo --no-fail-fast`: 12 tests passed.
  - `cargo check --workspace --tests`.
...
- Codec tests prove Taru-owned fields update from `NfoDocument`.
...
- Export service tests prove forced export preserves existing unknown sidecar
  fields while updating Taru-owned fields.
