...
`nako-api` now emits a checked Kotlin/JVM SDK entry at
`sdk/kotlin/src/main/kotlin/dev/nako/sdk/NakoClientSdk.kt`. The package is
included in the Android Gradle build as `:nako-public-client-sdk` and has local
...
`NakoPublicApiContract` wrapper has been deleted. Browse/library/item/search/person/genre/tag routes are built through generated
`NakoPublicClientRequests`; their success responses decode generated SDK DTOs
...
by generated `NakoPublicClientRequests`. User-playback progress/watched request
...
`NakoPublicApiContract`, `PublicApiUrl`, and route-text 404 classification were
...
- Android connection consumption initially went through `NakoPublicApiContract`
...
- Generated Kotlin enums now expose `wireValue` so Android adapters can map
  ...
  `@SerialName`.
- `NakoBrowseClient` now builds browse/library/item/search paths through
  generated `NakoPublicClientRequests`; 404 request classification is explicit
  and no longer pattern-matches path text.
- `listLibraries` now decodes `dev.nako.sdk.LibraryListResponse` and maps into
...
- AGKS-080 deletes the temporary generated-SDK facade
  `NakoPublicApiContract`; app code now imports generated constants and request
...
- `PublicApiUrl` and the old page/query/path encoding helpers were removed.
  App code uses generated descriptors plus the small `NakoRequestDescriptor.urlOn`
...
- Artwork requests now validate server-supplied image refs against generated
  `NakoPublicClientRequests.image(image.id)` descriptors instead of manually
