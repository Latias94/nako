...
--no-fail-fast` if the implementation touches shared `nako-core` APIs beyond
...
- `crates/nako-core/src/media/item.rs`
...
- Current duplicated merge anchors identified in `nako-metadata` and `nako-nfo`.
...
- `cargo nextest run -p nako-metadata merge --no-fail-fast` passed: 2 passed,
  26 skipped.
- `cargo nextest run -p nako-nfo nfo_service --no-fail-fast` passed: 16
  passed, 4 skipped.
...
- Shared `MetadataMergePolicy`, `MetadataMergeMode`, `MetadataLockScope`, and
  `populated_metadata_fields` now live in `crates/nako-core/src/media/merge.rs`.
- `nako-metadata` re-exports `MetadataMergePolicy` for compatibility with
...
- README and DESIGN now describe the shipped `nako-core` merge-policy boundary.
- TODO, HANDOFF, and WORKSTREAM state now point to MMP-050 closeout.
- Evidence anchors now use `crates/nako-core/src/media/merge.rs` instead of the
...
- `cargo fmt --all -- --check` passed.
- `git diff --check` passed.
- `cargo nextest run -p nako-metadata --no-fail-fast` passed: 28 passed, 0
  skipped.
- `cargo nextest run -p nako-nfo --no-fail-fast` passed: 20 passed, 0 skipped.
