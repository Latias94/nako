...
- [ ] MMP-020 [owner=unassigned] [deps=MMP-010] [scope=crates/taru-metadata,crates/taru-nfo]
  ...
  Handoff: Tests should describe the intended behavior before moving policy
  code.

## M2 - Shared Policy Boundary

- [ ] MMP-030 [owner=unassigned] [deps=MMP-020] [scope=crates/taru-core,crates/taru-metadata,crates/taru-nfo]
  ...
  Evidence: shared policy module path, `crates/taru-metadata/src/merge.rs`,
  `crates/taru-nfo/src/import.rs`.
  Handoff: Remove duplicated NFO field merge logic; keep workflow side effects
  in their existing crates.

## M3 - Integration And Documentation

- [ ] MMP-040 [owner=unassigned] [deps=MMP-030] [scope=docs,crates/taru-metadata,crates/taru-nfo]
  ...
  Handoff: Split provider priority configuration or merge diagnostics if they
  become necessary.

## M4 - Closeout

- [ ] MMP-050 [owner=planner] [deps=MMP-040] [scope=docs/workstreams/metadata-merge-policy-unification]
  Goal: Close the lane or split a narrower follow-up.
  Validation: `verify-rust-workstream` records fresh final gate evidence.
  Review: no blocking review findings.
  Evidence: `EVIDENCE_AND_GATES.md`, `WORKSTREAM.json`, `HANDOFF.md`.
  Handoff: Record remaining risks around provider priority, diagnostics, or
