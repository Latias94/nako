...
- Set `NAKO_TEST_POSTGRES_URL` to a test PostgreSQL database URL.
...
- A PostgreSQL gate may be skipped only when `NAKO_TEST_POSTGRES_URL` is not
...
- Ignored PostgreSQL contract tests must fail fast when
  `NAKO_TEST_POSTGRES_URL` is absent. A green ignored-test run without a real
...
- Recorded current state from M61:
  - `NakoDatabase` currently dispatches to SQLite only in production code.
  - `PostgresStore` is test-only and proves libraries/jobs/job leases.
  - `crates/nako-db/src/contract_tests.rs` has SQLite always-on and
...
- Added `DatabaseBackendKind::{Sqlite, Postgres}` and
  `DatabaseConnectOptions` to `nako-db`.
- Added facade-level backend identity and capability reporting through
  `NakoDatabase::backend_kind()` and `NakoDatabase::capabilities()`.
- Added `NakoDatabase::connect_with_options(...)` so production construction
  is explicit instead of URL-guessing-only.
- Kept SQLite as the default production and test backend.
- Added `NakoServerConfig.database_backend` with a default of `sqlite`.
- Updated `NakoApp::new` to pass explicit backend options into `NakoDatabase`.
...
cargo nextest run -p nako-db backend_kind backend_kind_rejects sqlite_connect_options nako_database_connect_options_select_sqlite_explicitly nako_database_rejects_postgres_until_contract_parity_is_complete nako_database_sqlite_persists_libraries
cargo nextest run -p nako-server config_round_trips_from_toml config_uses_default_runtime_settings config_accepts_explicit_postgres_backend_without_inferring_from_url
git diff --check
```
...
- Focused `nako-db` nextest passed: 6 passed, 66 skipped.
- Focused `nako-server` nextest passed: 3 passed, 171 skipped.
- `git diff --check` passed with Git CRLF normalization warnings only.
...
- Reworked `crates/nako-db/src/contract_tests.rs` around explicit contract
  metadata:
  - `ContractFamily`
  - `ContractSetup`
  - `ContractCase`
...
- `cargo nextest run -p nako-db contract --run-ignored ignored-only
  --no-fail-fast` passed in this environment, which means
  `NAKO_TEST_POSTGRES_URL` was available and the existing PostgreSQL proof
...
- Expanded `crates/nako-db/migrations/postgres/0001_contract_jobs.sql` with
...
- Implemented PostgreSQL `MediaRepository` and `LibraryItemRepository` methods
  required by the contract slice in `crates/nako-db/src/postgres.rs`.
...
cargo nextest run -p nako-db playback_runtime --run-ignored ignored-only --no-fail-fast
cargo nextest run -p nako-db contract --no-fail-fast
cargo nextest run -p nako-db contract --run-ignored ignored-only --no-fail-fast
git diff --check
```
...
- PostgreSQL persists transcode output paths because the server-owned playback
  ...
  remains owned by `nako-api`/HTTP boundaries and was not weakened.
...
- Focused PostgreSQL ignored event/addon/automation contract run passed: 3
  passed, 91 skipped.
- `nako-events` nextest passed: 3 passed, 0 skipped.
- `nako-automation` nextest passed: 3 passed, 0 skipped.
...
- Post-close AAD-090 update: Addon Side Effects now also require
  ...
  `NAKO_TEST_POSTGRES_URL` was not set in that session.
...
```bash
rg "impl ManagedArtworkRepository for PostgresStore|impl ArtworkCandidateRepository for PostgresStore|impl ArtworkTaskRepository for PostgresStore" crates/nako-db/src/postgres.rs -n
...
- Added a read-only `NakoApp::store()` accessor and retained the active
  `NakoDatabase` in app composition so HTTP diagnostics can report actual
...
- `runtime_supported` is derived from configured backend kind matching the
  active `NakoDatabase` backend. This keeps test/injected-store diagnostics
...
- Deleted `NakoDatabase::connect(&str)`, which silently selected SQLite and
  made production callers look URL-driven.
- Deleted `NakoDatabase::connect_with_sqlite_runtime(...)`, replacing the
  facade-level API shape with explicit `DatabaseConnectOptions` for production
  construction.
- Kept `NakoDatabase::connect_in_memory()` as a deliberately named test fixture
  convenience.
- Removed the remaining `nako-db` facade behavior-test imports of
...
```bash
rg -n "nako_db::sqlite|crate::sqlite|SqliteStore|sqlite::codec|store\\.sqlite\\(\\)" crates/nako-server/src crates/nako-api/src crates/nako-db/src/facade.rs crates/nako-db/src/tests.rs crates/nako-db/src/contract_tests.rs -g "*.rs"
rg -n "NakoDatabase::connect\\(|connect_with_sqlite_runtime|DatabaseConnectOptions::sqlite\\(" crates/nako-server/src crates/nako-db/src -g "*.rs"
rg -n "strftime|PRAGMA|rowid|json_extract|SqliteRow|INSERT OR" crates/nako-db/src crates/nako-server/src -g "*.rs" -g "!crates/nako-db/src/sqlite/**" -g "!crates/nako-db/src/postgres.rs"
rg -n "sqlite::memory:|sqlite://" crates/nako-server/src crates/nako-api/src -g "*.rs"
```

Result:

- No `nako_db::sqlite`, `sqlite::codec`, or `store.sqlite()` references remain
  in server/api code or facade-level tests.
- `SqliteStore` references outside `nako-db::sqlite` are confined to
  `NakoDatabase`'s internal facade dispatch.
...
- Remaining `sqlite::memory:` and `sqlite://nako.db` occurrences in
  `nako-server` are test fixture/default SQLite configuration values, not
...
- Promoted `PostgresStore` out of test-only compilation for the supported
  backend scope.
- Changed `NakoDatabase` from a SQLite field plus SQLite-only dispatch into an
...
cargo check -p nako-db --tests
cargo check -p nako-server --tests
cargo nextest run -p nako-db vfs_staging nako_database_postgres_runtime_capabilities_name_supported_and_split_surfaces --no-fail-fast
cargo nextest run -p nako-db postgres_vfs_staging --run-ignored ignored-only --no-fail-fast
NAKO_TEST_POSTGRES_URL=<local-test-url> cargo nextest run -p nako-db postgres_vfs_staging --run-ignored ignored-only --no-fail-fast
cargo nextest run -p nako-db contract --no-fail-fast
NAKO_TEST_POSTGRES_URL=<local-test-url> cargo nextest run -p nako-db contract --run-ignored ignored-only --no-fail-fast
cargo nextest run -p nako-server system_config --no-fail-fast
cargo check --workspace --tests
...
- Focused PostgreSQL ignored VFS/Staging nextest first failed when
  `NAKO_TEST_POSTGRES_URL` was unset, proving the harness no longer reports
  false green without a real PostgreSQL test URL.
- Focused PostgreSQL ignored VFS/Staging nextest passed with
  `NAKO_TEST_POSTGRES_URL` set to the local test database: 2 passed, 99
  skipped.
- `cargo fmt --all` passed.
- `cargo fmt --all -- --check` passed.
