...
2026-05-20 architecture review. Taru has no production compatibility burden
...
PostgreSQL-ready `taru-db` facade plus SQLite-owned adapter modules, backend
...
2. **Server composition and workflow ports** — keep `TaruApp` thin by grouping
...
4. **Metadata Candidate Graph** — let TMDB, Douban, Bangumi, NFO, Addons, and
   ...
   Metadata acceptance.
5. **Search semantics** — deepen `taru-search` from a shallow trait crate into
