...
- [x] FRA-020 [owner=codex] [deps=FRA-010] [scope=crates/nako-db,crates/nako-core,crates/nako-server,docs/adr,docs/workstreams/future-ready-architecture-refactor]
  ...
  Review: Must explicitly decide whether to split `nako-db` into
...
- [x] FRA-030 [owner=codex] [deps=FRA-020] [scope=crates/nako-core,crates/nako-db,crates/nako-server,docs]
  ...
  Validation: `cargo check -p nako-core --tests`; `cargo check -p nako-db --tests`;
  focused `cargo nextest run -p nako-db --no-fail-fast`; `git diff --check`.
  Review: Tests must prove behavior through the contract, not only through
  SQLite implementation details.
  Evidence: `crates/nako-db/src/contract_tests.rs`;
...
- [x] FRA-040 [owner=codex] [deps=FRA-030] [scope=crates/nako-db,crates/nako-server,crates/nako-library,crates/nako-metadata,crates/nako-nfo]
  ...
  options, and migrations behind `crates/nako-db/src/sqlite*`. Second slice
  introduced the public `NakoDatabase` facade, updated server and downstream
  ...
  facade, codec, contract tests, and SQLite-specific tests at the `nako-db`
  root at that point. Eighth slice moved the shared SQLite row-codec/helper
  module to `sqlite/codec.rs`, leaving the `nako-db` root with only
  ...
  import `SqliteStore`, SQLite codec helpers, `nako_core`, `nako_search`, and
  `sqlx` types explicitly, and moved the main `nako-db` behavior tests onto
  the `NakoDatabase` facade while keeping SQLite-only module tests inside the
  SQLite adapter. Root `nako-db` now exposes only `NakoDatabase` and
...
- [x] FRA-050 [owner=codex] [deps=FRA-040] [scope=crates/nako-core,crates/nako-db,crates/nako-server,crates/nako-library,crates/nako-metadata,crates/nako-nfo,crates/nako-automation,crates/nako-catalog,crates/nako-events,docs/adr,docs/workstreams/future-ready-architecture-refactor]
  ...
  `cargo nextest run -p nako-db migrations --no-fail-fast` or updated focused
...
- [x] FRA-060 [owner=codex] [deps=FRA-050] [scope=crates/nako-db,docs/workstreams/future-ready-architecture-refactor]
  ...
  always on, added ignored `NAKO_TEST_POSTGRES_URL` PostgreSQL job lease
...
- [x] FRA-070 [owner=codex] [deps=FRA-020] [scope=crates/nako-server/src/app,docs]
  Goal: Slim `NakoApp` by introducing cohesive runtime modules for storage,
  metadata, playback, automation/addons, admin operations, and jobs where they
  hide real construction complexity.
  Validation: `cargo check -p nako-server --tests`; focused server app/http
  ...
  Progress: Moved the `NakoApp::new_with_store` service/runtime wiring into
  `crates/nako-server/src/app/composition.rs`. `NakoApp` now keeps the public
  app surface and service accessors, while `NakoAppComposition`,
  `NakoAppServices`, and `NakoRuntimeResources` own construction of runtime
  ...
  registration. The old inline `NakoAppInner` construction path was deleted.
  Handoff: Continue with FRA-080 Local Inference Engine extraction.

- [x] FRA-080 [owner=codex] [deps=FRA-030] [scope=crates/nako-library,crates/nako-naming,crates/nako-core,docs]
  ...
  Validation: `cargo check -p nako-library --tests`; focused `cargo nextest run
  -p nako-library local_inference --no-fail-fast`; `git diff --check`.
  ...
  `LocalInferenceEngine` in `crates/nako-library/src/local_inference.rs` to
...
- [x] FRA-090 [owner=codex] [deps=FRA-030,FRA-080] [scope=crates/nako-metadata,crates/nako-core,crates/nako-nfo,docs]
  ...
  Validation: `cargo check -p nako-metadata --tests`; focused metadata nextest;
  ...
  `nako-core`, rewired `nako-metadata` provider search/fetch output to carry
...
- [x] FRA-100 [owner=codex] [deps=FRA-030] [scope=crates/nako-search,crates/nako-catalog,crates/nako-db,docs]
  Goal: Deepen `nako-search` into a semantic search module with explicit
  projection versioning, normalized fields, Browse Facets, Sort Keys, aliases,
  and provider identifiers.
  Validation: `cargo check -p nako-search --tests`; `cargo check -p
  nako-catalog --tests`; focused search/catalog nextest; `git diff --check`.
  ...
  Progress: Added explicit search projection semantics in `nako-core` and
  `nako-search`: projection version, Browse Facets, keyed facet kinds,
...
- [x] FRA-110 [owner=codex] [deps=FRA-070] [scope=crates/nako-api,crates/nako-server/src/http,docs/api,docs/workstreams/admin-api-typescript-contract]
  ...
  Progress: Audited `nako-api` Admin/Public/OpenAPI/SDK guardrails and
  `nako-server/src/http` job-returning routes. The generated Admin API
  contract still covers the eight read-model routes and the full `nako-api`
...
- [x] FRA-130 [owner=codex] [deps=FRA-040,FRA-070,FRA-080,FRA-090,FRA-100,FRA-110,FRA-120] [scope=workspace,docs]
  ...
  Progress: Deleted the `nako-api` root-level compatibility re-export shim,
  ...
  `openapi`, `sdk`, or `admin_contract`). Updated `nako-server` app, HTTP
