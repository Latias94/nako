...
- `cargo fmt --all -- --check`
- `cargo run -p taru-api --example emit-admin-typescript-contract -- --output apps/admin-web/src/adminApi/generated/contract.ts`
...
- AWAO-060 closeout:
  ...
  - `cargo nextest run -p taru-api admin_contract --no-fail-fast` — 5 passed,
    46 skipped.
  - `cargo check -p taru-api -p taru-server --tests`
  - `cargo nextest run -p taru-server addons --no-fail-fast` — 39 passed,
    212 skipped.
