...
- ATI-010 completed on 2026-05-20:
  - Added `TagListResponse` and `NakoBrowseClient.listTags`.
...
- ATI-020 completed on 2026-05-20:
  ...
  - `ClientBrowseDataSource.loadRelationshipIndex(Tags)` calls
    `NakoBrowseClient.listTags` and maps rows to stable Tag facet targets.
  ...
  - `RelationshipIndexRoute` presentation copy is family-aware for Genres and
    Tags.
  - Fresh validation:
    `apps\android\gradlew.bat -p apps\android :app:testDebugUnitTest --tests dev.nako.android.ui.screens.relationship.RelationshipIndexRouteTest --no-daemon --rerun-tasks`
    passed.
  - Fresh validation:
    `apps\android\gradlew.bat -p apps\android :app:testDebugUnitTest --tests "dev.nako.android.ui.browse.*" --no-daemon`
    passed.
...
- ATI-030 completed on 2026-05-20:
  - Added the Home Tags anchor next to Genres as a nested relationship index
    entry point.
  - `NakoBrowseShell` dispatches the Home Tags anchor through
  ...
  - Fresh validation:
    `apps\android\gradlew.bat -p apps\android :app:testDebugUnitTest --tests dev.nako.android.ui.browse.BrowseShellHostTest --no-daemon`
    passed.
  - Fresh validation:
    `apps\android\gradlew.bat -p apps\android :app:testDebugUnitTest --tests dev.nako.android.ui.browse.BrowseShellHostTest --tests dev.nako.android.ui.screens.relationship.RelationshipIndexRouteTest --no-daemon`
    passed.
  - Fresh validation:
    `apps\android\gradlew.bat -p apps\android :app:testDebugUnitTest --no-daemon`
    passed.
- ATI-040 completed on 2026-05-20:
  ...
  - Note: the fixture server build emitted pre-existing Rust warnings in
    `nako-server` unused code paths; the smoke gate itself passed.
