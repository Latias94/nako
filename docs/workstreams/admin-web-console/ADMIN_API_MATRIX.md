...
| Addons | `POST /admin/v1/addons`, `GET /admin/v1/addons`, `GET /admin/v1/addons/{addon_id}`, `PATCH /admin/v1/addons/{addon_id}/status`, `POST /admin/v1/addons/{addon_id}/unregister`, `POST /admin/v1/addons/{addon_id}/health-check`, `GET /admin/v1/addons/{addon_id}/surfaces`, token/grant routes under `/admin/v1/addons/{addon_id}` | Admin API v1 | Supports registration/list/detail/status filter with Admin DTO shielding, explicit enable/disable lifecycle mutation, terminal unregister with audit preservation, redaction-safe sidecar health checks, hosted surface read models, token issue/list/rotate/revoke, and grant replace/list. `admin-addon-operations-mvp` still reserves `POST /diagnostics/resource-call`. |
...
| Addons | Improved: `/admin/v1/addons` owns register/list/detail/status-filtered list, explicit enable/disable mutation, terminal unregister, health checks, hosted surface read models, token issue/list/rotate/revoke, and accepted permission grants without exposing raw persistence records. `admin-addon-operations-mvp` freezes the remaining route contract. | Implement remaining AAO route: resource-call diagnostics. Do not add Addon Manager process/package lifecycle here. |
