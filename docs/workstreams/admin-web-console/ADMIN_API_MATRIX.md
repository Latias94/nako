...
| Addons | `POST /admin/v1/addons`, `GET /admin/v1/addons`, `GET /admin/v1/addons/{addon_id}`, token/grant routes under `/admin/v1/addons/{addon_id}` | Admin API v1 | Supports registration/list/detail/status filter with Admin DTO shielding, token issue/list/rotate/revoke, and grant replace/list. Missing health check, delete/unregister, enable/disable patch route, hosted-page metadata, and resource-call diagnostics. |
...
| Addons | Improved: `/admin/v1/addons` owns register/list/detail/status-filtered list, token issue/list/rotate/revoke, and accepted permission grants without exposing raw persistence records. | Needs health check, enable/disable patch route if status update through full register is too coarse, delete/unregister, hosted-page metadata, and resource-call diagnostics. |
