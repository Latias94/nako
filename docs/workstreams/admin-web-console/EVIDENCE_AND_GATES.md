...
- `DESIGN.md` uses Nako domain language from `CONTEXT.md`.
...
- 2026-05-17: AWC-030 completed through ADR 0027. Admin-only routes should use
  `/admin/v1/*`, admin DTOs stay in `nako-api`, future Admin API contracts are
  generated separately from Public Client OpenAPI/SDK artifacts, and
  `nako-client-protocol` remains reserved for Public Client API concepts.
- 2026-05-17: Documentation gate passed with `git diff --check`; public
  protocol boundary verified by checking `crates/nako-client-protocol` had no
  changed files.
...
- 2026-05-17: AWC-035 / M52 completed. `GET /admin/v1/overview` is wired in
  `nako-server`, uses admin-owned DTOs in `nako-api::admin`, and returns safe
  ...
  validation passed: `cargo fmt --all -- --check`, `cargo check -p nako-api
  --tests`, `cargo nextest run -p nako-api --no-fail-fast` with 14 tests,
  `cargo check -p nako-server --tests`, `cargo nextest run -p nako-server
  http::tests::system --no-fail-fast` with 5 tests, `git diff --check`, and
  `git diff --name-only -- crates/nako-client-protocol` with no changed files.
