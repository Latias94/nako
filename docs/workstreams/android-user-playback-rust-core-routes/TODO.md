...
- [x] UPC-020 [owner=codex] [deps=UPC-010] [scope=crates/nako-client-core]
  ...
  body serialization.
  Evidence: `crates/nako-client-core/src/user_playback.rs`; tests.
  Handoff: DONE. Added `crates/nako-client-core/src/user_playback.rs`
...
- [x] UPC-030 [owner=codex] [deps=UPC-020] [scope=crates/nako-client-uniffi,scripts]
  Goal: Expose FFI-safe user-playback request builders over `nako-client-core`
  and keep `nako-client-uniffi` a thin binding adapter.
...
- [x] UPC-040 [owner=codex] [deps=UPC-030] [scope=apps/android/app/src/main/java/dev/nako/android/userplayback,apps/android/app/src/test/java/dev/nako/android/userplayback]
  Goal: Add Android `UserPlaybackCore` adapter over UniFFI and migrate
  `NakoUserPlaybackClient` route construction from `NakoPublicClientRequests`
  to Rust core descriptors while keeping generated SDK DTO/body mapping and
  Android diagnostics.
  Validation: `apps/android/gradlew.bat -p apps/android :app:testDebugUnitTest --tests dev.nako.android.userplayback.NakoUserPlaybackClientTest --no-daemon --rerun-tasks`
  ...
  may remain.
  Evidence: `NakoUserPlaybackClient.kt`; `RustUserPlaybackCore.kt`; tests.
  Handoff: DONE. Added Android `UserPlaybackCore`/`RustUserPlaybackCore`,
  migrated `NakoUserPlaybackClient` runtime route construction to Rust/UniFFI
