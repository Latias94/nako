...
- Select the next focused lane from the proposed provider-depth follow-ons or
  another architecture priority. Do not reopen closed Generated Artifact lanes
  or `metadata-provider-depth-and-precision` unless a new product requirement
  justifies a follow-on.
...
- [metadata-provider-depth-and-precision](metadata-provider-depth-and-precision/README.md):
  closed library-metadata-control-plane lane for TMDB series -> season provider
  graph preview, root-only refresh persistence guards, and explicit
  provider-depth follow-on routing.
...
- [transcode-interface-and-runtime-plan-deepening](transcode-interface-and-runtime-plan-deepening/README.md):
  ...
  HLS runtime plan Interface slice, low-level FFmpeg adapter exposure
  ratcheting, and HDR `HTP-030` unblock.
- [remote-storage-health-and-circuit-breaker](remote-storage-health-and-circuit-breaker/README.md):
  closed storage/VFS lane for adding durable **Storage Backend Health**, a
  backend circuit-breaker policy, redaction-safe Admin diagnostics, and
  operator circuit reset after the first storage resilience slice.
...
- [storage-vfs-resilience-and-source-identity](storage-vfs-resilience-and-source-identity/README.md):
  ...
  source identity behavior. Watcher/debounce, durable backend circuit breakers,
  hash escalation policy, and PostgreSQL runtime harness coverage are split as
  follow-ons.
...
- [hls-progressive-runtime-boundary](hls-progressive-runtime-boundary/README.md):
  ...
  and seek generation identity. It made HLS artifact publication, playlist
...
- [playback-subtitle-language-default-policy](playback-subtitle-language-default-policy/README.md):
  ...
  readiness, LL-HLS, DASH, DRM, and offline sync remain follow-ons.
- [playback-runtime-resource-scheduler](playback-runtime-resource-scheduler/README.md):
  ...
  HLS/remux permit enforcement and Admin pressure diagnostics before remote
  workers, queueing, LL-HLS, DASH, or DRM expand playback pressure.
