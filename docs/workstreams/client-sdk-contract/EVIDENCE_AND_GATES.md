...
- `nako-api` owns `public_client_paths()` locally in its AGPL OpenAPI module.
- `nako-client` owns a second `PUBLIC_CLIENT_PATHS` list.
...
- Keep `nako-client-protocol` Apache-2.0 and dependency-light.
- Make `nako-api` consume shared inventory without moving OpenAPI rendering out
  of the AGPL adapter crate.
- Make TypeScript generator consume the shared inventory through `nako-api`.
- Make `nako-client` consume shared inventory.
...
- Workstream docs define `nako-client-protocol` as the shared inventory owner,
  `nako-client` as the Rust SDK boundary, and `nako-api` as the AGPL
  OpenAPI/SDK generation adapter.

### CSC-020 Shared Public Route Inventory

- `crates/nako-client-protocol/src/lib.rs` defines `PublicClientRoute`,
...
- `nako-client-protocol` tests prove the inventory has 24 public paths, 20
...
- `crates/nako-api/src/openapi.rs` no longer owns a local public path list for
  OpenAPI route membership tests.
- `crates/nako-api/src/sdk.rs` consumes `nako_client_protocol::public_client_paths`
...
- `crates/nako-client/src/lib.rs` consumes
  `nako_client_protocol::public_client_paths`,
  `public_client_json_routes`, `public_client_streaming_routes`,
  `PUBLIC_CLIENT_ROUTES`, and `PublicClientRustSdkExposure`.
- Removed the local `nako-client` `PUBLIC_CLIENT_PATHS` duplication.
...
- `cargo tree -p nako-client-protocol`: passed; the protocol crate remains
  dependency-light with `serde` plus dev `serde_json`.
- `cargo tree -p nako-client`: passed; the SDK depends on protocol/runtime
  crates and does not depend on `nako-api` or server/internal Nako crates.
...
- Closeout validation on 2026-05-17:
  ...
  - `cargo check -p nako-api --tests`: passed.
  - `cargo check -p nako-client --tests`: passed.
  - `cargo nextest run -p nako-client-protocol --no-fail-fast`: 7 tests
    passed.
  - `cargo nextest run -p nako-api --no-fail-fast`: 11 tests passed.
  - `cargo nextest run -p nako-client --no-fail-fast`: 8 tests passed.
  ...
  - `cargo tree -p nako-client`: passed; no `nako-api` or server/internal
    Nako crate dependency.
  - `npm run check --prefix sdk/typescript`: passed.
...
- Eliminate route inventory duplication:
  `nako-api` and `nako-client` now consume protocol-owned inventory instead of
  owning local public path lists.
- Move route inventory into permissive protocol crate:
  `nako-client-protocol` owns `PUBLIC_CLIENT_ROUTES` and stays Apache-2.0.
- Keep OpenAPI and TypeScript SDK aligned:
  `nako-api` OpenAPI tests and `typescript_sdk()` use
  `nako_client_protocol::public_client_paths`.
- Add Rust SDK streaming request builders:
  `nako-client` exposes builders for direct stream GET, direct stream HEAD,
...
- Preserve license boundary:
  client-reusable inventory lives in `nako-client-protocol`, not only in AGPL
  `nako-api`; dependency trees confirm permissive crates do not depend on
  server/internal crates.

### Named Files And Artifacts

- `docs/workstreams/client-sdk-contract/`: workstream docs and closeout.
- `crates/nako-client-protocol/src/lib.rs`: route inventory source of truth.
- `crates/nako-api/src/openapi.rs`: consumes protocol inventory for OpenAPI
  path checks.
- `crates/nako-api/src/sdk.rs`: consumes protocol inventory for TypeScript SDK
  generation and tests.
- `crates/nako-client/src/lib.rs`: consumes protocol inventory and implements
  streaming request builders.
