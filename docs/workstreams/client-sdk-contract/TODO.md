...
- [x] CSC-020 [owner=codex] [deps=CSC-010] [scope=crates/taru-client-protocol, crates/taru-api]
  Goal: Move public client route inventory into `taru-client-protocol` and
  refactor `taru-api` OpenAPI/tests plus TypeScript generator to consume it.
  Validation: `cargo check -p taru-client-protocol --tests`,
  `cargo nextest run -p taru-client-protocol --no-fail-fast`,
  `cargo nextest run -p taru-api --no-fail-fast`.
  Evidence: protocol inventory types/constants and taru-api usage.
  Handoff: Keep OpenAPI rendering in `taru-api`; only neutral route facts move.

## M36.2 Rust SDK Inventory And Streaming Builders

- [x] CSC-030 [owner=codex] [deps=CSC-020] [scope=crates/taru-client]
  Goal: Make `taru-client` consume the shared inventory and add request
  builders for direct stream, HEAD preflight, remux stream, HLS playlist, and
  HLS segment routes.
  Validation: `cargo check -p taru-client --tests`,
...
- [x] CSC-040 [owner=codex] [deps=CSC-030] [scope=crates/taru-api, crates/taru-client, sdk/typescript]
...
- [x] CSC-050 [owner=planner] [deps=CSC-040] [scope=docs/api/HTTP_API.md, docs/GOALS.md, docs/ROADMAP.md, docs/workstreams/client-sdk-contract]
  ...
  taru-client-protocol`, `cargo tree -p taru-client`, `git diff --check`.
