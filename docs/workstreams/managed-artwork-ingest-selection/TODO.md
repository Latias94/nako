...
- [ ] MAIS-020 [owner=codex] [deps=MAIS-010] [scope=crates/taru-core,crates/taru-db,crates/taru-server,crates/taru-api,crates/taru-vfs,docs]
  ...
  `DESIGN.md`.
  Handoff: Continue with MAIS-030 only after fetch/cache/selection semantics
  and redaction rules are explicit.
...
- [ ] MAIS-030 [owner=codex] [deps=MAIS-020] [scope=crates/taru-core,crates/taru-db,crates/taru-server,crates/taru-api,crates/taru-vfs,docs/api]
  Goal: Implement the selected first candidate acceptance path through a
  Taru-owned managed artwork service or commit boundary.
  ...
  Review: verify no public response exposes raw candidate source URLs, Source
  Locators, filesystem paths, internal cache paths, remote storage handles, or
  unvalidated addon hotlinks.
