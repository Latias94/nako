...
| 2026-05-26 | BPAT-050 playback progress writes | `apps/admin-web/src/surfaces/media/MediaPages.tsx`, `apps/admin-web/src/surfaces/media/mediaSurface.test.tsx`; `cd apps/admin-web && npm run test -- mediaSurface.test.tsx`; `cd apps/admin-web && npm run test -- mediaDataSource.test.ts mediaSurface.test.tsx`; `cd apps/admin-web && npm run check`; `cd apps/admin-web && npm run build`; `rg -n "admin/v1|AdminApi|adminApi|source_locator|local path|ffmpeg|Bearer |Authorization" apps/admin-web/src/surfaces/media`; Playwright smoke at `http://127.0.0.1:4173/media/watch/item-episode-1?source_id=source-episode-1-alt` with fixture mode and simulated play/timeupdate/pause/ended events; `git diff --check` | PASS. Watch player events write User Playback State through the Public Client data source only after playback starts. Normal `timeupdate` writes are throttled by 30 seconds of playback-position delta, pause forces a progress flush, ended marks the selected source watched, and payloads include the selected `source_id`. Fixture smoke showed progress and watched visible-state updates while visible text still excluded `nako_bpt_fixture`, `/sources/`, `Bearer`, and `Authorization`; boundary grep matches remained test-only SDK header assertions. Vite build passed with existing chunk-size/plugin-timing warnings. |
| 2026-05-26 | BPAT-060 closeout | `python -m json.tool docs/workstreams/browser-playback-auth-transport/WORKSTREAM.json`; `git diff --check -- docs/workstreams/browser-playback-auth-transport docs/workstreams/README.md`; `cargo test -p nako-client-protocol public -- --nocapture`; `cargo test -p nako-client playback_decision_ticket_and_session_cancel_paths_are_stable -- --nocapture`; `cargo test -p nako-client sdk_inventory -- --nocapture`; `cargo test -p nako-api public_openapi -- --nocapture`; `cargo test -p nako-api sdk`; `cargo run -q -p nako-api --example emit-typescript-sdk -- --output sdk/typescript/src/index.ts`; `cargo nextest run -p nako-server browser_playback_ticket playback_ticket_bypass_is_limited_to_media_byte_routes source_ticket_is_opaque_and_validates_scope_and_expiry`; `cargo test -p nako-server playback -- --nocapture`; `cd apps/admin-web && npm run test -- mediaDataSource.test.ts mediaSurface.test.tsx`; `cd apps/admin-web && npm run check`; `cd apps/admin-web && npm run build`; `cargo fmt --all --check`; boundary grep; Playwright desktop/mobile smoke at `http://127.0.0.1:4173/media/watch/item-episode-1?source_id=source-episode-1-alt`; `git diff --check` | PASS. Final closeout gates prove the accepted browser transport contract, SDK/OpenAPI stability, server ticket validation, Media Web player/progress behavior, and mobile/desktop nonblank UI. The only boundary grep hits are test-only SDK Authorization assertions. Existing Vite chunk-size and plugin-timing warnings remain unrelated follow-ons. |
