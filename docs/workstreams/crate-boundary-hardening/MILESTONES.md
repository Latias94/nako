...
- Boundary audit of `nako-api`, `nako-core`, `nako-library`, `nako-nfo`,
  `nako-streaming`, `nako-transcode`, and `nako-server` playback surfaces.
...
`nako-api` becomes a server adapter over that boundary.

Deliverables:

- Create or solidify `nako-client-protocol` as the permissive public wire
  boundary.
- Move the first stable public response/request types out of `nako-api`.
- Keep `nako-api` as the AGPL mapping layer for server internals.
...
- focused `cargo nextest run -p nako-api`
- focused `cargo nextest run -p nako-client-protocol`
- `git diff --check`
...
- `cargo tree -p nako-client-protocol` shows no server-internal dependency.
...
Outcome: `nako-core` becomes easier to navigate because media and repository
concepts are grouped by concept instead of by historical accretion.

Deliverables:

- Split `crates/nako-core/src/media.rs` into concept-sized module files.
- Split `crates/nako-core/src/repository.rs` into narrower trait groupings if
...
Outcome: `nako-library` and `nako-nfo` expose clearer workflow modules for
scan, index, probe, codec, import, export, and orchestration code.

Deliverables:

- Split `crates/nako-library/src/lib.rs` into scan, index, probe, local
  inference, and summary modules.
- Split `crates/nako-nfo/src/lib.rs` into codec and workflow modules.
...
- `crates/nako-library/src/{summary,scan,index,probe,local_inference,failure}.rs`
- `crates/nako-nfo/src/{codec,summary,workflow,import,export}.rs`
...
- Clarify what belongs in `nako-streaming` versus `nako-transcode`.
- Keep `nako-server::app::playback` as orchestration and HTTP translation.
...
- `cargo fmt --all -- --check`
- `cargo check --workspace --tests`
- focused `cargo nextest run -p nako-streaming`
...
- `crates/nako-streaming/src/{selection,direct}.rs`
- `crates/nako-transcode/src/{plan,hardware,ffmpeg,session,runtime,remux,hls,runner_util}.rs`
