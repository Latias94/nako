...
- `nako-client-protocol` has stable public error codes, but not auth-specific
  codes yet.
...
- `nako-client-protocol` owns stable `unauthorized` and `forbidden` error
  codes.
- `NakoServerConfig` owns `[auth]` with auth enabled by default and
...
- `crates/nako-server/src/http/auth.rs` validates `Authorization: Bearer
...
- `docs/api/HTTP_API.md` documents bearer-token use, public health, and auth
  error behavior.
- `docs/development/LOCAL_SETUP.md` documents local `NAKO_ADMIN_TOKEN` setup
...
- `cargo nextest run -p nako-client-protocol --no-fail-fast`: passed, 4 tests.
- `cargo nextest run -p nako-server config --no-fail-fast`: passed, 11 tests.
- `cargo nextest run -p nako-server http::tests::system --no-fail-fast`:
  passed, 4 tests.
