...
```powershell
cargo fmt --package nako-client-core --package nako-client --check
...
```powershell
cargo fmt --package nako-client-protocol --package nako-api --package nako-client --check
...
- 2026-05-21: Completed `RCR-020` Android Rust/UniFFI build ergonomics.
  - Split `buildNakoClientUniFfiHost`, `generateNakoClientUniFfiKotlin`,
    `buildNakoClientUniFfiDebugAndroid`, and
    `buildNakoClientUniFfiReleaseAndroid`.
  ...
  - Added focused ABI selection with `-PnakoRustAndroidAbis=...`.
  - `apps/android/gradlew.bat -p apps/android :app:tasks --group "nako rust" --no-daemon`
    passed and listed the split tasks.
  - `apps/android/gradlew.bat -p apps/android :app:testDebugUnitTest --tests dev.nako.android.connection.NakoConnectionClientTest --no-daemon`
    passed. The output showed `buildNakoClientUniFfiHost` and
    `generateNakoClientUniFfiKotlin`, with no Android ABI build task.
  - `apps/android/gradlew.bat -p apps/android :app:assembleDebug -PnakoRustAndroidAbis=x86_64 --no-daemon`
    passed and ran `buildNakoClientUniFfiDebugAndroid` through the debug JNI
    packaging path.
  - `python -m json.tool docs/workstreams/android-rust-core-runtime-hardening/WORKSTREAM.json > $null`
...
- 2026-05-21: Completed `RCR-030` Rust client adapter reuse.
  - Added generic `CoreHttpRequestSpec`, `CoreQueryParam`,
    ...
    core request/response policy tests to `nako-client-core`.
  - `nako-client` now depends on `nako-client-core`, converts core request
    specs into reqwest requests, and maps core response-policy failures back
    into Rust client errors.
  - `cargo fmt --package nako-client-core --package nako-client --check`
    passed after formatting.
  - `cargo nextest run -p nako-client-core --no-fail-fast` passed with 9 tests.
...
- 2026-05-21: Completed `RCR-040` Rust public wire tolerance.
  ...
  - Updated `nako-client` remux query construction to use `wire_value()`.
  - Updated API/server fallout for non-`Copy` tolerant values.
  - `cargo fmt --package nako-client-protocol --package nako-api --package nako-client --check`
    passed.
  - `cargo nextest run -p nako-client-protocol --no-fail-fast` passed with 9 tests.
...
- 2026-05-21: Completed `RCR-050` Android playback core tracer.
  ...
  - `cargo fmt --package nako-client-core --package nako-client-uniffi --check`
    passed.
  - `cargo nextest run -p nako-client-core --no-fail-fast` passed with 11 tests.
  - `cargo nextest run -p nako-client-uniffi --no-fail-fast` passed with 2 tests.
