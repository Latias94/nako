...
- `taru-client-core` owns portable request construction, response/version/error
  policy, redaction-safe previews, playback target selection, and playback
  route/query construction.
- `taru-client-uniffi` remains a binding adapter over `taru-client-core`.
- `taru-client` remains the Rust async/reqwest adapter and consumes shared core
...
- Browse/catalog/user-playback Android route construction still uses the
  ...
  `taru-client-core` instead of adding new Kotlin-only policy.
...
2. Move browse/catalog route construction through `taru-client-core` once the
