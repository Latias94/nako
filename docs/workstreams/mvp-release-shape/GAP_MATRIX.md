...
Status: MRS-050 campaigns integrated
...
| --- | --- | --- | --- | --- |
| Active queue | Evidence-backed after campaign integration | `docs/architecture/LANES.md`; workstream inventory; active PTJCH/GAMA/CSAPA docs; `PTJCH-220` and `web-mvp-live-smoke` merge evidence. | `PTJCH-220` is integrated on `main` and no longer blocks the MVP cut. `GAMA-060` remains conditional. `CSAPA-050` remains P1 unless desktop-native playback is promoted. Known lane drift for GAMA/CSAPA must not be hidden. | Proceed to release-candidate gate execution or optional release-wrapper scripting. |
...
| Web browse/playback | Evidence-backed after campaign integration | `web-media-live-public-client-parity`; `web-mvp-live-smoke`; `web/src/test/mvp-live-smoke.test.tsx`; Public Client protocol/openapi inventory. | Browser/web is the accepted MVP client path. The deterministic Web MVP smoke now covers Public Client library browse/detail/playback ticket, native media/subtitle rendering, heartbeat, and unsafe text redaction. | Keep Gate 3 in the release ladder; no separate P0 Web split remains. |
| Playback runtime | Evidence-backed after campaign integration | `docs/architecture/PLAYBACK.md`; `playback-transcode-jellyfin-class-hardening`; `crates/nako-server/src/app/tests/playback.rs`. | `PTJCH-220` is integrated. Playback Runtime now owns HLS supersede candidate discovery, cancellation, bounded replacement admission, and superseded playback-session cancellation without moving transcode planning into the server. | Keep Gate 4 in the release ladder; route artifact I/O pressure to `PTJCH-310` or PAIP only if gates escalate it. |
...
| --- | --- | --- | --- | --- |
| `playback-transcode-jellyfin-class-hardening` | `PTJCH-310` | P0 resolved; follow-on active | `PTJCH-220` is reviewed, verified, and merged to `main`. Playback Runtime ownership is no longer an MVP blocker. `PTJCH-310` artifact I/O pressure remains a follow-on unless release gates escalate it. | Keep out of the MVP blocker list; run release-candidate playback gates before release closeout. |
...
| `web-product` live MVP smoke | `web-mvp-live-smoke` | P0 resolved | The deterministic Web/Public Client MVP smoke is implemented and merged to `main`. | Keep the Web smoke commands in Gate 3; broaden only if release candidate browser QA finds a real gap. |
