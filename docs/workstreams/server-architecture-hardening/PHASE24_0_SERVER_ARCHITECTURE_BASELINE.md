...
architecture: `NakoApp` composes runtime pieces, focused services own
...
- `crates/nako-server/src/app.rs`: root application type, service composition,
  library resolution helpers, startup work, and broad convenience methods.
- `crates/nako-server/src/app/metadata.rs`: metadata refresh, provider runtime
  construction, raw cache, and job orchestration.
- `crates/nako-server/src/app/playback.rs`: direct play, remux, HLS, staging,
  resource permits, and playback session orchestration.
- `crates/nako-server/src/app/storage.rs`: backend resolution, staging wrapper,
  and configured library storage behavior.
- `crates/nako-nfo/src/lib.rs`: NFO parse/export policy and current XML
  handling boundary.
- `crates/nako-catalog/src/lib.rs`: catalog graph hydration and search
...
- Make `NakoApp` thin enough that new feature work naturally lands in focused
  services.
- Introduce one lifecycle owner for background tasks before adding more
  scheduled or provider-driven work.
- Prefer deleting obsolete code over carrying compatibility shims while Nako is
  pre-compatibility.
...
1. Freeze the intended `NakoApp` public surface and identify methods that
...
3. Move remaining workflow orchestration out of `NakoApp` by bounded context.
...
- A supervisor abstraction can become too generic. It should solve concrete
  Nako lifecycle needs: startup, shutdown, cancellation, task failure, and
  diagnostics.
- Repository cleanup should not leak SQLite details into `nako-core` traits.
  Keep SQL-specific concerns inside `nako-db`.
