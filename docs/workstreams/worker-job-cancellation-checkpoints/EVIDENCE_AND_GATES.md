...
cargo nextest run -j 2 -p nako-server metadata --no-fail-fast
```
...
$env:CARGO_BUILD_JOBS='2'
$env:NEXTEST_TEST_THREADS='1'
cargo nextest run -j 2 -p nako-server background_scan_job_acknowledges_cancellation_before_probe_stage --no-fail-fast
...
- `WJCC-020`: Added runtime cancellation context support and a runtime test that
  ...
  `cargo check -p nako-server --tests`; `cargo fmt --all -- --check`;
...
- `WJCC-030` (2026-05-19): Migrated metadata maintenance to the
  ...
  `cargo check -j 2 -p nako-api -p nako-server --tests` (pass);
  `cargo nextest run -j 2 -p nako-server metadata_maintenance_job_acknowledges_cancellation_before_next_item --no-fail-fast`
  (1 passed);
  `cargo nextest run -j 2 -p nako-server job_runtime --no-fail-fast`
  (5 passed);
  `cargo nextest run -j 2 -p nako-server job_cancel --no-fail-fast`
  (1 passed);
  `cargo nextest run -j 2 -p nako-server runtime --no-fail-fast` (16 passed);
  `cargo nextest run -j 2 -p nako-server metadata --no-fail-fast` (20 passed);
  ...
  `cargo check -j 2 -p nako-server --tests` (pass) and
  `cargo nextest run -j 2 -p nako-server runtime --no-fail-fast` (16 passed).
- `WJCC-040` (2026-05-19): Migrated library scan and NFO import/export jobs to
  ...
  `nako-nfo` API follow-on. Gates run sequentially with `CARGO_BUILD_JOBS=2`
  and `NEXTEST_TEST_THREADS=1`:
  `cargo check -j 2 -p nako-server --tests` (pass);
  `cargo nextest run -j 2 -p nako-server background_scan_job_acknowledges_cancellation_before_probe_stage --no-fail-fast`
  (1 passed);
  `cargo nextest run -j 2 -p nako-server nfo --no-fail-fast` (9 passed);
  `cargo nextest run -j 2 -p nako-server job_runtime --no-fail-fast`
  (5 passed);
  `cargo fmt --all -- --check` (pass);
