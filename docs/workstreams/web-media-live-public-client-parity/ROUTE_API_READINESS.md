...
| Player entry | Local `VideoPlayer` mock exists. | Ready for contract design: `getPlaybackDecision`, `createBrowserPlaybackTicket`, `getPlaybackSession`, `heartbeatPlaybackSession`, `cancelPlaybackSession`, `hlsSegment`, raw stream/remux/HLS helpers, and browser ticket URL DTOs exist. | WMLP-040 should wire browser-ticket playback from a detail source ID and assert that bearer tokens are not embedded in media URLs. |
