...
```powershell
cargo check -j 2 -p nako-server -p nako-api --tests
cargo nextest run -j 2 -p nako-server managed_artwork_variant --no-fail-fast
cargo nextest run -j 2 -p nako-api managed_artwork_variant --no-fail-fast
git diff --check
```
...
```powershell
cargo check -j 2 -p nako-server --tests
cargo nextest run -j 2 -p nako-server "managed_artwork|artifact|lifecycle|remediation|drift" --no-fail-fast
git diff --check
```
...
```powershell
cargo check -j 2 -p nako-server -p nako-db -p nako-api --tests
cargo nextest run -j 2 -p nako-server artwork --no-fail-fast
...
```powershell
rg -n "storage_uri|managed-artwork://|source_uri|cache_uri|content_hash|local_path|artifact_root" crates/nako-api crates/nako-server/src/http crates/nako-server/src/app docs/api
```
...
- Fresh validation:
  ...
  - `cargo check -j 2 -p nako-server -p nako-api --tests`: passed.
  - `cargo nextest run -j 2 -p nako-server managed_artwork_variant --no-fail-fast`:
...
- Extracted local Managed Artwork Artifact storage and inventory into
  `crates/nako-server/src/app/artwork/artifact_store.rs`.
- App-layer change:
  ...
  - `ManagedArtworkAppService` owns projection from internal store issues to
    Admin storage-drift DTO reasons, keeping `nako-api` DTOs out of the file
    storage Module.
- Fresh validation:
  - `cargo fmt --all -- --check`: passed.
  - `cargo check -j 2 -p nako-server --tests`: passed.
  - `cargo nextest run -j 2 -p nako-server managed_artwork_variant --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_process_next_managed_artwork_ingest_stores_internal_artifact_without_public_artwork --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_managed_artwork_cleanup_removes_only_unselected_artifacts_without_locator_leaks --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_managed_artwork_storage_drift_reports_missing_and_stray_files_without_locator_leaks --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_managed_artwork_remediation_requires_confirmation_and_deletes_only_untracked_artifact_files --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server "admin_managed_artwork_(cleanup|storage_drift|remediation)" --no-fail-fast`:
...
- Extracted Managed Artwork ingest execution into
  `crates/nako-server/src/app/artwork/ingest_pipeline.rs`.
- App-layer change:
...
- Fresh validation:
  ...
  - `cargo nextest run -j 2 -p nako-server admin_process_next_managed_artwork_ingest_stores_internal_artifact_without_public_artwork --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_process_next_managed_artwork_ingest_fails_with_redacted_safe_summary_for_unsupported_media_type --no-fail-fast`:
    passed; 1 test passed.
  - `cargo nextest run -j 2 -p nako-server admin_process_next_managed_artwork_ingest_fails_with_redacted_safe_summary_for_invalid_image --no-fail-fast`:
    passed; 1 test passed.
  - `git diff --check`: passed with Git line-ending notices only.
...
- Split the first SQLite Managed Artwork adapter concerns:
  - `crates/nako-db/src/artwork/gallery.rs` now owns Admin gallery SQL, query
    helpers, and row mapping;
  - `crates/nako-db/src/artwork/lifecycle.rs` now owns artifact lifecycle SQL,
    lifecycle summary aggregation, and lifecycle row helpers;
  - `crates/nako-db/src/artwork.rs` keeps the repository trait implementation
    and routes through those modules.
- Fresh validation:
  - `cargo fmt --all -- --check`: passed.
  - `cargo check -j 2 -p nako-db --tests`: passed.
  - `cargo nextest run -j 2 -p nako-db artwork --no-fail-fast`: passed; 11
    tests passed.
  - `git diff --check`: passed with Git line-ending notices only.
...
- Split Selected Artwork SQLite adapter concern:
  - `crates/nako-db/src/artwork/selected.rs` now owns Selected Artwork get/list
    SQL and publication/unpublication transactions;
  - `crates/nako-db/src/artwork.rs` keeps repository trait methods and routes
...
- Completed the SQLite Managed Artwork adapter concern split:
  - `crates/nako-db/src/artwork/candidate.rs` now owns Artwork Candidate
    repository methods, candidate lookup SQL, and status update helpers;
  - `crates/nako-db/src/artwork/ingest.rs` now owns Managed Artwork Ingest
    lookup/insert helpers and job transaction helpers used by ingest state
    transitions;
  - `crates/nako-db/src/artwork/artifact.rs` now owns Managed Artwork Artifact
    insert/get helpers and artifact SQL;
  - `crates/nako-db/src/artwork/lifecycle.rs` now also owns the unselected
    artifact cleanup transaction, keeping lifecycle diagnostics and cleanup in
    one concern;
  - `crates/nako-db/src/artwork.rs` keeps the existing repository trait methods
...
- Audited Managed Artwork API DTO locality:
  - Managed Artwork Admin DTOs, conversion helpers, and redaction tests were
    concentrated in `crates/nako-api/src/admin.rs` alongside unrelated Admin
    DTOs;
  - moved them into `crates/nako-api/src/admin/managed_artwork.rs`;
  - kept `admin.rs` re-exporting the module so `nako_api::*` and HTTP callers
...
- Fresh validation:
  - `cargo fmt --all -- --check`: passed.
  - `cargo check -j 2 -p nako-api --tests`: passed.
  - `cargo check -j 2 -p nako-server --tests`: passed.
