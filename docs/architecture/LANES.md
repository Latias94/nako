...
| storage-vfs | Idle after cache repair diagnostics and source fingerprint escalation, hash execution, scheduling diagnostics, durable job contract/summary, internal enqueue, and queued planner first slices. | Consider source fingerprint hash scheduler/executor/operator integration, playback artifact/source-read pressure, scan scheduling, or PostgreSQL runtime harness work. | Coordinate with playback input staging, library scan/probe lanes, and control-plane durable job policy before queue-backed work. |
...
contract/summary, internal enqueue, and queued planner first slices. Open a
Trellis task before starting cache repair, source fingerprint hash
scheduler/executor/operator integration, playback artifact I/O pressure, scan
