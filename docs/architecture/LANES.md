...
| storage-vfs | Idle after cache repair diagnostics and the source fingerprint escalation policy seam. | Consider source fingerprint hash execution / operator diagnostics, playback artifact/source-read pressure, scan scheduling, or PostgreSQL runtime harness work. | Coordinate with playback input staging and library scan/probe lanes. |
...
source fingerprint hash execution / operator diagnostics, playback artifact I/O
