...
| storage-vfs | Idle after cache repair diagnostics and the source fingerprint hash execution kernel. | Consider source fingerprint hash scheduling / operator diagnostics, playback artifact/source-read pressure, scan scheduling, or PostgreSQL runtime harness work. | Coordinate with playback input staging and library scan/probe lanes. |
...
escalation policy seam. Open a Trellis task before starting cache repair,
source fingerprint hash scheduling / operator diagnostics, playback artifact I/O
