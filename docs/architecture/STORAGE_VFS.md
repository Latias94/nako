...
internal target enqueue seam, and internal single-job executor command shipped
as of 2026-06-07.
...
- the internal single-job executor command claims one explicit
  ...
  payload material;
- purge/delete/invalidation, durable repair scheduler execution, Admin API
  enqueue routes, backend configuration mutation, library file writes, and retry
  queues remain out of this shipped boundary;
...
- `proposed:vfs-cache-repair-durable-remediation`: durable repair scheduler
  execution, Admin enqueue and retry/requeue routes,
  ...
  shipped internal enqueue and single-job executor seams.
