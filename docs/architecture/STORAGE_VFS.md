...
| Remote storage boundary | Shipped durable health foundation plus focused PostgreSQL storage/source parity evidence | `docs/adr/0016-remote-storage-and-vfs-cache-boundary.md`; `docs/workstreams/storage-vfs-resilience-and-source-identity/`; `docs/workstreams/remote-storage-health-and-circuit-breaker/`; `.trellis/tasks/archive/2026-06/06-02-01d-hls-artifact-io-pressure-enforcement/`; `.trellis/tasks/archive/2026-06/06-06-06-06-overnight-fearless-refactor-development-plan/` | Open follow-ons for cache repair, broader scan scheduling, or broader PostgreSQL runtime harness work. |
...
| VFS cache | Shipped diagnostics foundation, action preview, latest-failure refresh, action plan, target-scoped preview, selected-target refresh execution, read-only remediation plan, durable job contract, internal target enqueue seam, internal single-job executor command, Admin manual enqueue/execute/retry routes, Admin Jobs diagnostics projection, automation policy dry-run planner, explicit internal automation enqueue command, disabled-by-default recurring automation enqueue runtime, disk-scan scheduler integration, and internal retry seam | `docs/adr/0016-remote-storage-and-vfs-cache-boundary.md`; `docs/workstreams/storage-vfs-resilience-and-source-identity/`; `.trellis/tasks/archive/2026-06/06-04-06-04-vfs-cache-repair-action-preview-first-slice/`; `.trellis/tasks/archive/2026-06/06-04-vfs-cache-repair-operator-actions/`; `.trellis/tasks/archive/2026-06/06-04-vfs-cache-uri-scoped-previews/`; `.trellis/tasks/archive/2026-06/06-04-vfs-cache-repair-executable-refresh-action/`; `.trellis/tasks/archive/2026-06/06-06-vfs-cache-repair-non-destructive-remediation-plan-first-slice/`; `.trellis/tasks/archive/2026-06/06-06-06-06-overnight-fearless-refactor-development-plan/`; `.trellis/tasks/archive/2026-06/06-07-vfs-cache-repair-job-diagnostics-projection/`; `.trellis/tasks/archive/2026-06/06-07-06-07-vfs-cache-repair-automation-policy-planner/`; `crates/nako-core/src/vfs_cache.rs`; `crates/nako-server/src/app/storage.rs`; `crates/nako-server/src/app/vfs_cache_repair_runtime.rs`; `crates/nako-server/src/app/jobs.rs`; `crates/nako-server/src/app/runtime.rs`; `crates/nako-server/src/http/admin.rs` | Cache purge/delete/invalidation, backend configuration mutation, library file writes, destructive automatic repair actions, and realtime/incident diagnostics remain follow-ons. |
...
manual enqueue/execute routes, disk-scan scheduler integration, internal retry
seam, and Admin manual retry route shipped as of 2026-06-07.
...
- read-only remediation planning groups unresolved repair pressure by safe
  recommended action and classification, exposes only aggregate counts and
  bounded opaque target samples, and points refreshable groups to the existing
  selected-target refresh route without executing it;
- `JobKind::VfsCacheRepair` and
  `VfsCacheRepairJobInput` define the first durable repair queue contract with
  action, source scheme, operation, failure timestamp/count, URI digest, and
  stored failure authority, without raw URI, local path, backend URL,
  credential, raw backend error, etag, fingerprint, or cache payload material;
- internal target enqueue accepts only unresolved targets whose diagnostic
  recommends `refresh_cache`, writes `storage.vfs.cache_repair` jobs under the
  existing `disk.scan` runtime budget, and does not refresh cache entries or
  touch backend configuration during enqueue;
- enqueue is idempotent for queued/running jobs with the same validated input,
  scanning paginated durable job results so large queues cannot hide an existing
  duplicate, while terminal jobs do not block future enqueue;
- the internal single-job executor command claims one explicit
  `JobKind::VfsCacheRepair` job through `DurableJobRuntime`, reloads the current
  unresolved failure by safe durable input match, reuses selected-target refresh
  authority, and persists a redaction-safe summary without raw URI, local path,
  backend URL, credential, raw backend error, etag, fingerprint, or cache
  payload material;
- Admin manual enqueue exposes
  `POST /admin/v1/storage/vfs-cache/repair/targets/{target_ref}/jobs`, accepts
  only the opaque selected-target reference plus optional priority, and returns
  only redaction-safe job facts plus whether the job was newly enqueued or
  already queued;
- Admin manual execution exposes
...
- the internal retry seam can create a new queued retry from a failed
  `VfsCacheRepair` job after validating kind, resource class, redaction-safe
  input, job bindings, failed status, and a still-unresolved refresh target;
- VFS cache repair retry preserves the failed source job for audit, copies safe
  durable input/resource/priority/library bindings through
  `JobRepository::enqueue_job_retry`, canonicalizes delayed retry timestamps to
  UTC RFC3339, and continues due retries through the existing disk-scan
  scheduler path;
- Admin manual retry exposes
  `POST /admin/v1/storage/vfs-cache/repair/jobs/{job_id}/retry`, accepts only
  `max_attempts` and `next_attempt_at`, delegates validation to the storage app
  service, and returns only the same redaction-safe `AdminJobListItem` shape as
  other durable job commands;
...
- the internal automation policy dry-run planner classifies unresolved repair
  ...
  policy, reports a non-mutating automation boundary, and does not enqueue jobs,
  refresh cache, purge/delete/invalidate cache entries, mutate backend
  configuration, or write library files;
- the explicit internal automation enqueue command reuses the dry-run policy
  report and target-scoped durable enqueue authority to queue eligible repair
  jobs with redaction-safe job facts, without executing jobs or touching storage
  backends;
- the disabled-by-default recurring automation runtime runs under
  `RuntimeSupervisor`, periodically reuses the explicit automation enqueue
  authority for `refresh_cache` targets only, reports safe eligible/blocked and
  enqueued/already-queued counts, and leaves execution on the existing disk-scan
  scheduler path;
- purge/delete/invalidation, backend configuration mutation, library file
  writes, destructive automatic repair actions, and realtime/incident
  diagnostics remain out of this shipped boundary;
...
- `proposed:vfs-cache-repair-automation-and-mutation-policy`: cache
  purge/delete/invalidation semantics, backend configuration workflows,
  destructive automatic repair actions, broader repair workers, and
  realtime/incident diagnostics beyond the shipped dry-run automation planner,
  disabled recurring enqueue runtime, Admin manual enqueue/execute/retry,
  disk-scan scheduler, and Admin Jobs diagnostics projection seams.
