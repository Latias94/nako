...
| --- | --- | --- | --- | --- |
| Product-Operator readiness overview | `GET /admin/v1/overview` exposes a bounded `operator_readiness` checklist for setup, Media Library scan, playback, storage, network, and backup posture. Admin Web Overview renders the same live facts and only shows existing Admin route targets as read-only action references. | Focused overview route tests and Admin Web Overview route tests from `06-10-media-server-gap-analysis` U1. | Shipped | Use this as the first M1 triage entry point. It aggregates existing safe diagnostics and does not add repair mutations, hidden jobs, or raw path/command exposure. |
