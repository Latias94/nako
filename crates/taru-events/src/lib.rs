...
use taru_core::{
    EventOutboxRepository, Result, TaruError, WebhookDeliveryAttemptId,
    ...
    T: WebhookTransport,
{
        ...
        R: EventOutboxRepository + WebhookRepository,
    {
        ...
        let attempt = repository
            .create_webhook_delivery_attempt(taru_core::NewWebhookDeliveryAttempt {
...
pub fn build_webhook_request(
    ...
    let payload =
        serde_json::from_str(&event.payload_json).map_err(|err| TaruError::InvalidInput {
    ...
    let body = serde_json::to_string(&envelope).map_err(|err| TaruError::InvalidInput {
        message: format!("failed to serialize webhook event envelope: {err}"),
    })?;
    let mut headers = vec![
        ("content-type".to_owned(), "application/json".to_owned()),
        ("x-taru-event-id".to_owned(), event.id.to_string()),
        (
            "x-taru-event-kind".to_owned(),
...
fn webhook_error(error: reqwest::Error) -> TaruError {
    TaruError::Provider {
...
mod tests {
    ...
    async fn capture_webhook(
        ...
        let event_id = header_value(&headers, "x-taru-event-id");
        let signature = header_value(&headers, "x-taru-signature");
        ...
        let response = transport
            .post(WebhookHttpRequest {
                ...
                headers: vec![
                    ("content-type".to_owned(), "application/json".to_owned()),
                    (
                        "x-taru-event-id".to_owned(),
                        "018f0000-0000-7000-8000-000000000001".to_owned(),
                    ),
                    ("x-taru-signature".to_owned(), "sha256=test".to_owned()),
                ],
    ...
    async fn webhook_delivery_signs_and_persists_successful_attempt() {
        ...
        assert!(
            request
                .headers
                .iter()
                .any(|(name, value)| name == "x-taru-signature" && value.starts_with("sha256="))
        );
        assert!(!request.body.contains("secret"));
        assert!(!request.body.contains("TARU_WEBHOOK_SECRET"));
    }
