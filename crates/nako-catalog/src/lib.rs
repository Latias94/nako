...
use nako_core::{BrowseFacet, BrowseFacetKind};
use nako_core::{
    ...
    MediaRepository, MetadataSource, NakoError, PageRequest, Person, PersonId,
    ProviderMappingRepository, ProviderMappingStatus, ProviderSubject, Result, SortKey,
    SortKeyKind, Studio, StudioId, Tag, TagId,
};
...
struct CatalogHydrationSnapshot {
    pub item: MediaItem,
    pub sources: Vec<nako_core::MediaSource>,
...
fn resolve_studio(
    lookup: &CatalogHydrationLookup,
    studio_ref: &nako_core::StudioRef,
    name: String,
    source: &MetadataSource,
) -> Studio {
    for external_id in non_empty_external_ids(&studio_ref.external_ids) {
...
async fn list_all_item_sources<R>(
    repository: &R,
    item_id: MediaItemId,
) -> Result<Vec<nako_core::MediaSource>>
where
    R: MediaRepository,
{
    let mut offset = 0;
