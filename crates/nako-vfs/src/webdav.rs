...
impl WebDavSecretResolver for EnvWebDavSecretResolver {
    fn resolve(&self, env_name: &str) -> Result<String> {
        env::var(env_name).map_err(|err| NakoError::InvalidInput {
...
impl WebDavBackend {
    ...
    pub fn new_with_client_and_resolver(
        ...
        let base_url = Url::parse(&config.base_url).map_err(|err| NakoError::InvalidInput {
...
mod tests {
    ...
    async fn webdav_backend_stages_file_to_deterministic_local_path() {
        ...
        assert!(!first.reused);
        assert_eq!(tokio::fs::read(&first.path).await.unwrap(), b"nako");
