...
impl StorageBackendHealthRepository for SqliteStore {
    async fn upsert_storage_backend_health(
        ...
        sqlx::query(storage_backend_health_upsert_sql())
            ...
            .bind(record.library_id.map(|id| id.to_string()))
            .bind(&record.scheme)
    ...
    async fn list_storage_backend_health(
        ...
        let rows = sqlx::query(&format!(
            ...
            WHERE (?1 IS NULL OR library_id = ?1)
              ...
              AND (?4 IS NULL OR circuit_breaker_state = ?4)
...
fn row_to_storage_backend_health_record(row: SqliteRow) -> Result<StorageBackendHealthRecord> {
