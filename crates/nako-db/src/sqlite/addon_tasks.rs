...
const ADDON_TASK_RUN_SELECT: &str = r#"
            SELECT
                ...
                jobs.retry_of_job_id AS job_retry_of_job_id,
                jobs.next_attempt_at,
                jobs.queued_at,
                jobs.started_at,
                jobs.completed_at,
                jobs.cancel_requested_at,
                addon_task_runs.addon_id,
                ...
                addon_task_runs.created_at,
                addon_task_runs.updated_at
...
impl AddonTaskRunRepository for SqliteStore {
    ...
    async fn list_addon_task_runs(
        ...
        let mut query = QueryBuilder::new(ADDON_TASK_RUN_SELECT);
        query.push(" WHERE 1 = 1");

        if let Some(addon_id) = filter.addon_id {
            query.push(" AND addon_task_runs.addon_id = ");
            query.push_bind(addon_id.to_string());
        }
        if let Some(declaration_id) = filter.declaration_id {
            query.push(" AND addon_task_runs.declaration_id = ");
            query.push_bind(declaration_id);
        }
        if let Some(library_id) = filter.library_id {
            query.push(" AND jobs.library_id = ");
            query.push_bind(library_id.to_string());
        }
        if let Some(source_id) = filter.source_id {
            query.push(" AND jobs.source_id = ");
            query.push_bind(source_id.to_string());
        }

        query.push(" ORDER BY jobs.queued_at DESC, jobs.id DESC LIMIT ");
    ...
    async fn claim_next_addon_task_run(
        ...
        let job_id = sqlx::query_scalar::<_, String>(
            ...
            WHERE jobs.status = ?1
                ...
                AND (?4 IS NULL OR addon_task_runs.declaration_id = ?4)
                AND (?5 IS NULL OR jobs.id = ?5)
        ...
        .bind(request.addon_id.to_string())
        .bind(request.declaration_id.as_deref())
        .bind(request.job_id.map(|id| id.to_string()))
    ...
    async fn report_addon_task_run_progress(
        ...
        .bind(guard.job_id.to_string())
        .bind(guard.run_token.to_string())
...
async fn insert_job_tx(transaction: &mut sqlx::Transaction<'_, Sqlite>, job: NewJob) -> Result<()> {
    sqlx::query(
        r#"
        INSERT INTO jobs (
            id,
...
fn row_to_addon_task_run(row: SqliteRow) -> Result<AddonTaskRunRecord> {
...
fn row_to_leased_addon_task_run(row: SqliteRow) -> Result<LeasedAddonTaskRun> {
    let job_id: JobId = parse_id(row_get::<String>(&row, "id")?)?;
