        ...
        let rows = sqlx::query(
            ...
            FROM (
                SELECT
                    ...
                    batch_items.batch_id::text AS batch_id,
                    batch_items.artifact_id::text AS artifact_id,
                    batch_items.status AS batch_item_status,
                    batch_items.outcome_id::text AS outcome_id,
                    outcomes.status AS outcome_status,
                    outcomes.item_id::text AS item_id,
                    batch_items.plan_item_json::text AS plan_item_json,
                    outcomes.plan_json::text AS outcome_plan_json,
                    ...
                    to_char(batch_items.created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS created_at,
                    to_char(batch_items.updated_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS updated_at
                ...
                WHERE batch_items.status <> 'pending'
                    AND (
                        $1::text IS NULL
                        OR ($1::text = 'needs_repair' AND batch_items.status IN ('stale', 'failed'))
                        OR ($1::text = 'needs_review' AND batch_items.status = 'skipped')
                        OR ($1::text = 'replay_only' AND batch_items.status = 'noop')
                        OR ($1::text = 'resolved' AND batch_items.status = 'applied')
                    )
                UNION ALL
                SELECT
                    'apply_outcome' AS source,
                    NULL AS batch_id,
                    outcomes.artifact_id::text AS artifact_id,
                    NULL AS batch_item_status,
                    outcomes.id::text AS outcome_id,
                    outcomes.status AS outcome_status,
                    outcomes.item_id::text AS item_id,
                    ...
                    to_char(outcomes.created_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS created_at,
                    to_char(outcomes.updated_at AT TIME ZONE 'UTC', 'YYYY-MM-DD"T"HH24:MI:SS.MS"Z"') AS updated_at
                ...
                WHERE NOT EXISTS (
                    ...
                    AND (
                        $1::text IS NULL
                        OR ($1::text = 'needs_repair' AND outcomes.status = 'failed')
                        OR ($1::text = 'replay_only' AND outcomes.status = 'noop')
                        OR ($1::text = 'resolved' AND outcomes.status = 'applied')
                    )
            ) AS recovery_entries
