...
#[async_trait::async_trait]
impl UserPlaybackProfileRepository for PostgresStore {
    async fn create_user_playback_profile(
        ...
        self.get_user_playback_profile(&profile.principal_id, profile.profile_id)
            ...
            .ok_or_else(|| NakoError::Database {
                message: format!(
                    "user playback profile {} for principal {} was not found after PostgreSQL insert",
    ...
    async fn get_user_playback_profile(
        ...
        let query =
            format!("{USER_PLAYBACK_PROFILE_SELECT} WHERE principal_id = $1 AND profile_id = $2");
        let row = sqlx::query(&query)
            .bind(principal_id.as_str())
    ...
    async fn get_default_user_playback_profile(
        ...
        let query = format!(
            ...
            WHERE principal_id = $1 AND is_default = true
            "#
        );
        let row = sqlx::query(&query)
            .bind(principal_id.as_str())
...
fn row_to_user_playback_profile(row: &PgRow) -> Result<UserPlaybackProfile> {
...
async fn clear_user_playback_profile_defaults(
    ...
    .bind(principal_id.as_str())
    .bind(except_profile_id.map(|id| id.as_uuid()))
    .bind(updated_at_ms)
    .execute(&mut **transaction)
    .await
    .map_err(database_error)?;
