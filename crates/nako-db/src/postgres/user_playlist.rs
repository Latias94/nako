...
impl UserPlaylistRepository for PostgresStore {
    async fn create_user_playlist(&self, playlist: NewUserPlaylist) -> Result<UserPlaylistRecord> {
        ...
        self.get_user_playlist(&playlist.principal_id, playlist.id)
            ...
            .ok_or_else(|| NakoError::Database {
                message: format!(
                    "user playlist {} for principal {} was not found after PostgreSQL insert",
    ...
    async fn get_user_playlist(
        ...
        get_user_playlist(self, principal_id, playlist_id).await
    }
    ...
    async fn update_user_playlist_name(
        ...
        .bind(update.playlist_id.as_uuid())
...
fn user_playlist_summary_query(principal: &AuthenticatedPrincipal) -> QueryBuilder<'_, Postgres> {
...
fn row_to_user_playlist(row: PgRow) -> Result<UserPlaylistRecord> {
    ...
    Ok(UserPlaylistRecord {
        ...
        visibility: UserPlaylistVisibility::parse(&visibility).ok_or_else(|| {
            NakoError::Database {
                message: format!(
                    "unknown user playlist visibility stored in PostgreSQL: {visibility}"
                ),
            }
        })?,
...
fn row_to_user_playlist_summary_projection(row: PgRow) -> Result<UserPlaylistSummaryProjection> {
