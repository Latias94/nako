...
#[tokio::test]
async fn nako_database_connect_options_select_sqlite_explicitly() {
    let store =
        NakoDatabase::connect_with_options(DatabaseConnectOptions::sqlite("sqlite::memory:"))
            .await
            .unwrap();
...
#[tokio::test]
async fn nako_database_postgres_runtime_capabilities_include_managed_artwork() {
    assert_eq!(
...
#[tokio::test]
async fn nako_database_postgres_selection_enters_runtime_connection_path() {
    let err =
        NakoDatabase::connect_with_options(DatabaseConnectOptions::postgres("not-a-postgres-url"))
            .await
            .unwrap_err();
...
#[tokio::test]
async fn nako_database_sqlite_persists_libraries() {
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_library_profiles() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_media_items_and_sources() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_persists_user_playback_state_by_principal() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_continue_watching_filters_watched_zero_and_other_principals() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_video_item_hierarchy_and_multiple_sources() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_tracks_source_less_library_items() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_transcode_sessions() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_transcode_sessions_with_filters_and_pagination() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_marks_stale_transcode_sessions_failed() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_metadata_policy_records() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_metadata_provider_attempts() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    let attempt = NewMetadataProviderAttempt {
        id: nako_core::MetadataProviderAttemptId::new(),
        job_id: job.id,
        item_id: item.id,
        provider: ExternalProvider::Tmdb,
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_provider_subjects_and_mappings() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_local_inference_evidence_without_confirming_metadata() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_rolls_back_catalog_graph_when_search_projection_commit_fails() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_media_probe_results() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_vfs_cache_records_and_failures() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_catalog_graph_records() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_scan_state_search_and_artwork_tasks() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_job_lifecycle() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_jobs_with_filters_and_pagination() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_marks_running_jobs_failed_on_startup_and_preserves_queued_jobs() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_outbox_events_idempotently() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_outbox_events_with_filters_and_pagination() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_webhook_endpoint_and_attempts() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    let endpoint = store
        .upsert_webhook_endpoint(NewWebhookEndpoint {
            ...
            secret_env: Some("NAKO_TEST_WEBHOOK_SECRET".to_owned()),
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_automation_provider_and_artifacts() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_generated_artifact_proposals_with_readiness() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_marks_generated_artifact_proposal_stale_after_target_changes() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_addon_registration() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_addon_tokens_and_grants() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_unregisters_addon_registration_atomically() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    let active_token = store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_active".to_owned(),
    ...
    let revoked_token = store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_revoked".to_owned(),
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_addon_side_effects_idempotently() {
    ...
    let store = NakoDatabase::from_sqlite_for_tests(sqlite);
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_runtime".to_owned(),
...
#[tokio::test]
async fn nako_database_sqlite_records_addon_side_effect_apply_outcome() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_round_trips_addon_artwork_candidates_idempotently() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_artwork".to_owned(),
...
#[tokio::test]
async fn nako_database_sqlite_accepts_artwork_candidate_into_managed_ingest_atomically() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    let library_id = LibraryId::new();
    store
...
#[tokio::test]
async fn nako_database_sqlite_managed_artwork_ingest_requeue_resets_failed_ingest_and_job() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_requeue".to_owned(),
    ...
    let running_requeue = store
        ...
        .await
        .unwrap_err();
    assert!(matches!(running_requeue, NakoError::Conflict { .. }));
    ...
    let stored_requeue = store
        ...
        .await
        .unwrap_err();
    assert!(matches!(stored_requeue, NakoError::Conflict { .. }));
}

#[tokio::test]
async fn nako_database_sqlite_managed_artwork_startup_recovery_fails_only_claimed_ingests() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_recovery".to_owned(),
...
#[tokio::test]
async fn nako_database_sqlite_publishes_stored_managed_artifact_as_selected_artwork_idempotently() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_art".to_owned(),
            token_hash: "sha256:artwork".to_owned(),
        })
        .await
        .unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_publishes_selected_artwork_with_item_kind_guard() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    let wrong_kind = store
        ...
        .await
        .unwrap_err();
    assert!(matches!(wrong_kind, NakoError::Conflict { .. }));
...
#[tokio::test]
async fn nako_database_sqlite_selected_artwork_unpublish_without_deleting_artifact() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_managed_artwork_gallery_for_item_without_raw_locators() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_lists_managed_artwork_lifecycle_with_selected_artwork_protection() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_cleanup_marks_only_unselected_managed_artwork_artifacts_deleted() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
...
#[tokio::test]
async fn nako_database_sqlite_rejects_addon_token_rotation_across_addons() {
    let store = NakoDatabase::connect_in_memory().await.unwrap();
    store.migrate().await.unwrap();
    ...
    store
        .create_addon_token(NewAddonToken {
            ...
            token_prefix: "nako_at_first".to_owned(),
    ...
    let error = store
        .rotate_addon_token(
            ...
            NewAddonToken {
                ...
                token_prefix: "nako_at_wrong".to_owned(),
    ...
    assert!(matches!(error, NakoError::Conflict { .. }));
    assert_eq!(
        store
