...
pub const REFERENCE_ADDON_ID: &str = "nako.reference.metadata";

#[must_use]
pub fn reference_manifest(base_url: impl Into<String>) -> AddonManifest {
    AddonManifest {
        id: REFERENCE_ADDON_ID.to_owned(),
        name: "Nako Reference Metadata Addon".to_owned(),
        version: "0.1.0".to_owned(),
        protocol_version: ADDON_PROTOCOL_VERSION.to_owned(),
        base_url: base_url.into(),
        description: Some("Minimal metadata suggestion addon for Nako protocol tests".to_owned()),
...
async fn health(Json(request): Json<AddonHealthCheckRequest>) -> Json<AddonHealthCheckResponse> {
    Json(AddonHealthCheckResponse {
        ...
        diagnostics: serde_json::json!({"fixture": "nako-reference-addon"}),
    })
}
