...
pub(crate) fn version_failure(
    ...
    let observed = header_value(response, nako_client_protocol::API_VERSION_HEADER)?;
    if observed == nako_client_protocol::CLIENT_PROTOCOL_VERSION {
        return None;
    }
    Some(CoreRuntimeFailure {
...
fn public_error(response: &CoreHttpResponse, secrets: &[&str]) -> Option<CorePublicError> {
    let error =
        serde_json::from_str::<nako_client_protocol::ErrorResponse>(&response.body_utf8).ok()?;
