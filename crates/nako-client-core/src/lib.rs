...
mod tests {
    ...
    fn input() -> CoreConnectionProbeInput {
        CoreConnectionProbeInput::new("https://nako.example/api/", "secret-token")
    }

    fn api_header() -> CoreHttpHeader {
        CoreHttpHeader::new(
            nako_client_protocol::API_VERSION_HEADER,
            nako_client_protocol::CLIENT_PROTOCOL_VERSION,
        )
    }
    ...
    fn connection_probe_starts_with_unauthenticated_health_request() {
        ...
        assert_eq!(request.safe_preview.url, "https://nako.example/api/health");
    }
    ...
    fn generic_core_request_builds_encoded_url_auth_header_and_safe_preview() {
        ...
        assert_eq!(
            request.url,
            "https://nako.example/api/sources/source%201/playback/decision?direct_play=true&container=mp4%2Cwebm"
        );
        assert_eq!(
            request.headers,
    ...
    fn generic_response_interpreter_preserves_public_error_and_version_failures() {
        let request = build_core_request(
            &CoreHttpRequestSpec::new("demo", "https://nako.example", "GET", "/demo")
                .access_token(Some("secret-token".to_owned())),
        )
        .safe_preview;
    ...
    fn playback_session_requests_use_core_methods_paths_auth_and_redaction() {
        ...
        assert_eq!(
            inspect.url,
            "https://nako.example/api/playback/sessions/session%2F1"
        );
        assert_eq!(
        ...
        assert_eq!(
            cancel.url,
            "https://nako.example/api/playback/sessions/session%201/cancel"
        );
        assert_eq!(
            cancel.headers,
            vec![CoreHttpHeader::new("Authorization", "Bearer secret-token")]
    ...
    fn playback_targets_follow_mode_without_auth_or_media3_policy() {
        ...
        assert_eq!(
            ...
            "https://nako.example/api/sources/source%201/stream"
        );
    ...
    fn connection_probe_reports_missing_token_before_auth_probe() {
        let outcome =
            start_connection_probe(&CoreConnectionProbeInput::new("https://nako.example", "  "));
    ...
    fn health_success_advances_to_redacted_authenticated_probe_request() {
        ...
        assert_eq!(
            request.url,
            "https://nako.example/api/libraries?limit=1&offset=0"
        );
        assert_eq!(
            request.headers,
            vec![CoreHttpHeader::new("Authorization", "Bearer secret-token")]
    ...
    fn browse_request_builders_use_stable_paths_pagination_auth_and_redaction() {
        ...
        assert_eq!(
            library_sources.url,
            "https://nako.example/api/libraries/library%201/sources?limit=24&offset=0"
        );
        ...
        assert_eq!(item.url, "https://nako.example/api/items/item%2F1");
    }
    ...
    fn browse_facet_and_search_builders_encode_ids_facets_and_page() {
        ...
        assert_eq!(
            genre_items.url,
            "https://nako.example/api/genres/genre%201/items?limit=24&offset=12"
        );
        ...
        assert_eq!(
            tag_items.url,
            "https://nako.example/api/tags/tag%3Afavorite/items?limit=10&offset=0"
        );
        ...
        assert_eq!(
            person_items.url,
            "https://nako.example/api/people/person%201/items"
        );
    ...
    fn artwork_image_request_uses_core_auth_variants_and_safe_preview() {
        ...
        assert_eq!(unscaled.url, "https://nako.example/api/images/poster%2F1");
    }
    ...
    fn user_playback_read_builders_use_stable_paths_pagination_auth_and_redaction() {
        ...
        assert_eq!(
            state.url,
            "https://nako.example/api/users/me/playback-state/items/item%201"
        );
        assert_eq!(
            state.headers,
            vec![CoreHttpHeader::new("Authorization", "Bearer secret-token")]
    ...
    fn user_playback_write_builders_use_json_put_body_and_item_encoding() {
        ...
        assert_eq!(
            progress.url,
            "https://nako.example/api/users/me/playback-state/items/item%2F1/progress"
        );
        assert_eq!(
            progress.headers,
            vec![
