...
use crate::{
    ...
    MediaItemId, MediaSourceId, NakoError, Result, SecretString,
};

pub const ADDON_TOKEN_RAW_PREFIX: &str = "nako_at_";
...
mod tests {
    ...
    fn addon_principal_allows_global_or_matching_library_grants() {
        ...
        let principal = AddonPrincipal {
            ...
            token: AddonTokenRecord {
                ...
                token_prefix: "nako_at_prefix".to_owned(),
    ...
    fn addon_manifest_fingerprint_redacts_and_changes_with_manifest() {
        let first = AddonManifestFingerprint::new(r#"{"id":"example","secret":"nako_at_secret"}"#);
        let second = AddonManifestFingerprint::new(r#"{"id":"example","secret":"changed"}"#);

        assert!(first.as_str().starts_with("sha256:"));
        assert_ne!(first, second);
        assert!(!first.as_str().contains("nako_at_secret"));
        assert_eq!(
