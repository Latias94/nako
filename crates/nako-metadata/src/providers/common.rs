...
use nako_core::{ExternalProvider, ImageKind, ImageRef, NakoError, Result, SecretString};
use reqwest::header::{AUTHORIZATION, HeaderMap, HeaderName, HeaderValue};
use time::{OffsetDateTime, format_description::well_known::Rfc3339};

use super::TMDB_PROVIDER_NAME;
pub(crate) fn provider_request_error(provider: &str, error: reqwest::Error) -> NakoError {
...
pub(crate) fn tmdb_parse_error(operation: &str, error: impl ToString) -> NakoError {
...
pub(crate) fn header_map_from_pairs(pairs: &[(String, SecretString)]) -> Result<HeaderMap> {
    ...
    for (name, value) in pairs {
        let name =
            HeaderName::from_bytes(name.as_bytes()).map_err(|err| NakoError::InvalidInput {
