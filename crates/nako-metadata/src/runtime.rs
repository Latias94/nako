...
use nako_core::{NakoError, Result, SecretString};
...
impl MetadataHttpRuntime {
    pub fn new(config: MetadataHttpRuntimeConfig) -> Result<Self> {
        ...
        let client = builder.build().map_err(|err| NakoError::InvalidInput {
        ...
        F: Fn() -> reqwest::RequestBuilder,
    {
        ...
        for attempt in 1..=attempts {
            ...
            let response = match send_result {
                ...
                Err(_) => {
                    last_error = Some(NakoError::Provider {
                        provider: provider.to_owned(),
                        message: format!(
        ...
        let error = last_error.unwrap_or_else(|| NakoError::Provider {
            provider: provider.to_owned(),
