use nako_core::{
    ...
    MetadataProviderAttemptStatus, MetadataProviderErrorClass, NakoError,
    NewMetadataProviderAttempt, ProviderRawResponse, Result,
};
...
pub(super) struct MetadataProviderRefreshSuccess {
    ...
    refresh_mode: nako_core::MetadataRefreshMode,
    updated: bool,
}
...
impl MetadataProviderRefreshError {
    pub(super) fn into_error(self) -> NakoError {
        match self {
            Self::NoMatch(message) => NakoError::NotFound {
                entity: "metadata_candidate",
                id: message,
            },
            Self::ProviderFailed(message) => NakoError::Provider {
...
fn classify_provider_failure_message(message: &str) -> MetadataProviderErrorClass {
    classify_provider_error_class(&NakoError::Provider {
        provider: "metadata_provider".to_owned(),
...
fn classify_provider_error(error: NakoError) -> MetadataProviderRefreshError {
    match error {
        NakoError::NotFound { .. } => MetadataProviderRefreshError::NoMatch(error.to_string()),
        ...
        NakoError::Storage { .. } | NakoError::Database { .. } => {
            MetadataProviderRefreshError::Fatal(error)
        }
    }
}

fn classify_provider_error_class(error: &NakoError) -> MetadataProviderErrorClass {
    match error {
        NakoError::NotFound { .. } => MetadataProviderErrorClass::NoMatch,
        NakoError::Unsupported(_) => MetadataProviderErrorClass::Unsupported,
        NakoError::InvalidInput { .. } | NakoError::Conflict { .. } => {
            MetadataProviderErrorClass::Unknown
        }
        NakoError::Unauthorized { .. } | NakoError::Forbidden { .. } => {
            MetadataProviderErrorClass::Auth
        }
        NakoError::Provider { message, .. } => {
