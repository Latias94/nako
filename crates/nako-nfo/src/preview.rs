use nako_core::{LocalMetadataPolicy, MediaKind, MediaRepository, NakoError, Result};
...
pub struct NfoAuthorityPreviewRequest {
    pub library_id: nako_core::LibraryId,
...
pub struct NfoAuthorityPreviewDecision {
    pub source_id: nako_core::MediaSourceId,
            ...
            Ok(metadata) => {
                    ...
                    .contains(StorageCapabilities::WRITABLE)
                {
                    return preview_failure(
                        ...
                        NakoError::Unsupported("NFO export requires a writable sidecar target"),
                    );
                }
...
fn classify_preview_read_failure(err: &NakoError) -> NfoAuthorityPreviewReason {
    match err {
        NakoError::Unsupported(_) => NfoAuthorityPreviewReason::StorageUnsupported,
