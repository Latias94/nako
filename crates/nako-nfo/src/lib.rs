...
mod tests {
    ...
    use nako_core::{
        ...
        MediaSourceId, MetadataField, MetadataFieldLock, MetadataSource, NakoError, PageRequest,
  ...
  <genre>Action</genre>
        ...
        assert_eq!(
            ...
            nako_core::MetadataCandidateSource::Nfo
        );
...
"#,
    ...
    async fn nfo_service_forced_export_reports_backup_retention_pruning() {
        ...
        for index in 0..5 {
            fs::write(
                movies.join(format!("demo.nfo.nako-backup-000{index}")),
                format!("old backup {index}"),
            )
            .unwrap();
        }
        fs::write(movies.join("other.nfo.nako-backup-0000"), "other").unwrap();
        ...
        assert!(movies.join("demo.nfo.nako-backup-0001").exists());
        assert!(movies.join("other.nfo.nako-backup-0000").exists());
...
"#,
    ...
    async fn nfo_service_rejects_export_without_write_sidecar_policy() {
        ...
        assert_eq!(
            err,
            NakoError::Unsupported("NFO export requires write-sidecar local metadata policy")
        );
    }
    ...
    impl StorageBackend for RecordingStorageBackend {
        ...
        async fn list(&self, _uri: &StorageUri) -> nako_core::Result<Vec<ObjectMetadata>> {
            Ok(Vec::new())
        }
        ...
        async fn write(
        ...
        ) -> nako_core::Result<StorageWriteReport> {
            ...
            if self.fail_backup && request.backup.mode == StorageBackupMode::ExistingFile {
                return Err(NakoError::storage_backup(
                ...
                && self.files.lock().unwrap().contains_key(&request.uri)
            {
                Some(StorageBackupReport {
                    ...
                    backup_uri: StorageUri::parse(format!("{}.nako-backup-test", request.uri))
                        .unwrap(),
    ...
    async fn seed_item(store: &NakoDatabase, library_id: LibraryId, locator: &str) -> MediaItem {
        seed_item_with_metadata(
            store,
            library_id,
            locator,
