use nako_core::{
    ...
    MediaKind, MetadataCandidateGraph, MetadataCandidateRecord, NakoError, Result,
};
...
impl NfoCodec for MovieNfoCodec {
    fn parse(&self, xml: &str) -> Result<NfoDocument> {
        let document = roxmltree::Document::parse(xml).map_err(|err| NakoError::InvalidInput {
    ...
    fn render_preserving(
        ...
        let parsed =
            roxmltree::Document::parse(existing_xml).map_err(|err| NakoError::InvalidInput {
                message: format!("invalid NFO XML: {err}"),
            })?;
...
fn required_child_text(node: XmlNode<'_, '_>, name: &str) -> Result<String> {
    optional_child_text(node, name).ok_or_else(|| NakoError::InvalidInput {
