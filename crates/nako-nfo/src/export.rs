use nako_core::{
    ...
    MediaSource, MetadataRepository, NakoError, ProviderMappingRepository, Result,
    StorageErrorKind,
};
use nako_search::SearchIndex;
use nako_vfs::{StorageBackend, StorageBackupPolicy, StorageWriteRequest};
...
fn classify_read_failure(err: &NakoError) -> NfoFailureKind {
    match err {
        NakoError::Storage { .. } => NfoFailureKind::StorageRead,
        NakoError::Unsupported(_) => NfoFailureKind::StorageUnsupported,
        _ => NfoFailureKind::Unknown,
    }
}

fn classify_preservation_failure(err: &NakoError) -> NfoFailureKind {
    match err {
        NakoError::InvalidInput { .. } => NfoFailureKind::NfoPreservation,
        NakoError::Conflict { .. } => NfoFailureKind::NfoConflict,
        NakoError::Unsupported(_) => NfoFailureKind::NfoPreservation,
        _ => NfoFailureKind::Unknown,
    }
}

fn classify_render_failure(err: &NakoError) -> NfoFailureKind {
    match err {
        NakoError::InvalidInput { .. } => NfoFailureKind::NfoRender,
        _ => NfoFailureKind::Unknown,
    }
}

fn classify_write_failure(err: &NakoError) -> NfoFailureKind {
    match err {
        NakoError::Storage {
            kind: StorageErrorKind::Backup,
            ..
        } => NfoFailureKind::StorageBackup,
        NakoError::Storage { .. } => NfoFailureKind::StorageWrite,
        NakoError::Unsupported(_) => NfoFailureKind::StorageUnsupported,
        _ => NfoFailureKind::Unknown,
    }
}
