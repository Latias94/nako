...
impl ArtworkTaskRepository for PostgresStore {
    async fn enqueue_artwork_task(&self, _task: &ArtworkTask) -> Result<()> {
        Err(postgres_unsupported("Artwork Tasks"))
    }

    async fn get_artwork_task(&self, _id: ArtworkTaskId) -> Result<Option<ArtworkTask>> {
        Err(postgres_unsupported("Artwork Tasks"))
    }

    async fn list_artwork_tasks(&self, _page: PageRequest) -> Result<Vec<ArtworkTask>> {
        Err(postgres_unsupported("Artwork Tasks"))
    }
}
...
impl ArtworkCandidateRepository for PostgresStore {
    async fn create_artwork_candidate(
        &self,
        _candidate: NewArtworkCandidate,
    ) -> Result<ArtworkCandidateRecord> {
        Err(postgres_unsupported("Managed Artwork"))
    }

    async fn get_artwork_candidate(
        &self,
        _id: ArtworkCandidateId,
    ...
    async fn set_artwork_candidate_status(
        &self,
        _id: ArtworkCandidateId,
        _status: ArtworkCandidateStatus,
    ) -> Result<ArtworkCandidateRecord> {
        Err(postgres_unsupported("Managed Artwork"))
    }
    ...
    async fn find_artwork_candidate_by_source(
        ...
        _kind: &ImageKind,
        _source_kind: ArtworkCandidateSourceKind,
        _source_uri: &str,
    ) -> Result<Option<ArtworkCandidateRecord>> {
        Err(postgres_unsupported("Managed Artwork"))
    }
...
impl ManagedArtworkRepository for PostgresStore {
    async fn accept_managed_artwork_candidate_ingest(
        &self,
        _candidate_id: ArtworkCandidateId,
        _ingest: NewManagedArtworkIngest,
        _job: NewJob,
    ...
    async fn commit_managed_artwork_artifact(
        &self,
        _ingest_id: ManagedArtworkIngestId,
        _artifact: NewManagedArtworkArtifact,
        _job_summary_json: Option<String>,
    ...
    async fn publish_selected_artwork(
        &self,
        _artifact_id: ManagedArtworkArtifactId,
    ...
    async fn list_managed_artwork_artifact_lifecycle(
        &self,
        _filter: ManagedArtworkArtifactLifecycleFilter,
        _page: PageRequest,
...
fn postgres_unsupported(feature: &'static str) -> TaruError {
    TaruError::Unsupported(match feature {
        "Managed Artwork" => {
            "PostgreSQL backend does not support Managed Artwork until the managed-artwork-postgresql-parity follow-on lands"
        }
        "Artwork Tasks" => {
            "PostgreSQL backend does not support Artwork Tasks until the managed-artwork-postgresql-parity follow-on lands"
        }
        _ => "PostgreSQL backend does not support this repository surface",
    })
}
