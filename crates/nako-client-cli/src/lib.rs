...
#[command(name = "nako-client-cli")]
#[command(about = "Nako public client CLI")]
pub struct Cli {
...
fn build_client(
    cli: &Cli,
    transport: impl ClientTransport + 'static,
) -> Result<NakoClient, CliError> {
    let mut client = NakoClient::with_transport(&cli.base_url, transport)?;
...
async fn execute(client: NakoClient, command: Command) -> Result<String, CliError> {
