...
const HLS_AUDIO_GROUP_ID: &str = "nako-audio";
...
pub(super) fn author_hls_entry_playlist(
    ...
    if manifest.media_renditions().is_empty() {
...
fn author_single_variant_master_playlist(manifest: &HlsArtifactManifest) -> Result<String> {
    ...
    lines.extend(audio_media_lines(manifest));
    lines.extend(subtitle_media_lines(manifest));
    lines.push(format!(
        "#EXT-X-STREAM-INF:BANDWIDTH={SINGLE_VARIANT_FALLBACK_BANDWIDTH}{}",
        stream_inf_media_attributes(manifest)
    ));
...
fn author_adaptive_master_playlist(body: &str, manifest: &HlsArtifactManifest) -> String {
    let mut media_lines = audio_media_lines(manifest);
    media_lines.extend(subtitle_media_lines(manifest));
    ...
    for line in body.lines() {
        ...
        authored.push(author_master_playlist_line(line, manifest));
    }
...
fn author_master_playlist_line(line: &str, manifest: &HlsArtifactManifest) -> String {
    ...
    let mut authored = content.to_owned();
    if manifest.media_renditions().has_audios() && !content.contains("AUDIO=") {
        authored.push_str(&format!(",AUDIO=\"{HLS_AUDIO_GROUP_ID}\""));
    }
    if manifest.media_renditions().has_subtitles() && !content.contains("SUBTITLES=") {
        authored.push_str(&format!(",SUBTITLES=\"{HLS_SUBTITLE_GROUP_ID}\""));
    }

    format!("{authored}{trailing}")
}

fn stream_inf_media_attributes(manifest: &HlsArtifactManifest) -> String {
    let mut attributes = Vec::new();
    if manifest.media_renditions().has_audios() {
        attributes.push(format!("AUDIO=\"{HLS_AUDIO_GROUP_ID}\""));
    }
    if manifest.media_renditions().has_subtitles() {
        attributes.push(format!("SUBTITLES=\"{HLS_SUBTITLE_GROUP_ID}\""));
    }
...
fn audio_media_lines(manifest: &HlsArtifactManifest) -> Vec<String> {
    manifest
        .media_renditions()
        .audios()
        .iter()
        ...
        .map(|(position, audio)| {
            ...
            let default = if audio.default { "YES" } else { "NO" };
            let mut attributes = vec![
                "TYPE=AUDIO".to_owned(),
                format!("GROUP-ID=\"{}\"", hls_attribute_value(HLS_AUDIO_GROUP_ID)),
            ...
            attributes.push(format!("URI=\"{}\"", audio.playlist_file_name()));
...
mod tests {
    use super::*;
    use nako_transcode::{
        HlsAudioRendition, HlsMediaRenditionPlan, HlsOutputRequirement, HlsRendition,
        HlsSegmentContainer, HlsSubtitleRendition,
    };
    ...
    #[test]
    fn author_hls_entry_playlist_generates_single_variant_master_for_audio() {
        let manifest = single_variant_manifest_with_audio();
        let body = "#EXTM3U\n#EXTINF:1,\nsegment_00000.ts\n#EXT-X-ENDLIST\n";

        let authored = author_hls_entry_playlist(body, &manifest).unwrap();

        assert!(authored.starts_with("#EXTM3U\n#EXT-X-VERSION:3\n#EXT-X-MEDIA:"));
        assert!(authored.contains("TYPE=AUDIO"));
        assert!(authored.contains("GROUP-ID=\"nako-audio\""));
        assert!(authored.contains("NAME=\"eng\""));
        assert!(authored.contains("DEFAULT=YES"));
        assert!(authored.contains("LANGUAGE=\"eng\""));
        assert!(authored.contains("URI=\"audio_0.m3u8\""));
        assert!(authored.contains("AUDIO=\"nako-audio\"\nplaylist.m3u8"));
        assert!(!authored.contains("SUBTITLES=\"nako-subtitles\""));
        assert!(!authored.contains("segment_00000.ts"));
        assert!(authored.ends_with('\n'));
    }
    ...
    #[test]
    fn author_hls_entry_playlist_enriches_adaptive_master_for_audio_and_subtitles() {
        let manifest = adaptive_manifest_with_audio_and_subtitles();
        let body = "#EXTM3U\n#EXT-X-VERSION:7\n#EXT-X-STREAM-INF:BANDWIDTH=3128000,RESOLUTION=1280x720\nvariant_0.m3u8\n";

        let authored = author_hls_entry_playlist(body, &manifest).unwrap();

        assert!(authored.starts_with("#EXTM3U\n#EXT-X-VERSION:7\n#EXT-X-MEDIA:TYPE=AUDIO"));
        assert!(authored.contains("#EXT-X-MEDIA:TYPE=AUDIO"));
        assert!(authored.contains("GROUP-ID=\"nako-audio\""));
        assert!(authored.contains("URI=\"audio_0.m3u8\""));
        assert!(authored.contains("#EXT-X-MEDIA:TYPE=SUBTITLES"));
        assert!(authored.contains("URI=\"subtitle_0.m3u8\""));
        assert!(authored.contains(
            "#EXT-X-STREAM-INF:BANDWIDTH=3128000,RESOLUTION=1280x720,AUDIO=\"nako-audio\",SUBTITLES=\"nako-subtitles\""
        ));
    }
    ...
    fn single_variant_manifest_with_audio() -> HlsArtifactManifest {
        HlsArtifactManifest::single_variant(
            "hls",
            "hls/playlist.m3u8",
            "hls/segment_%05d.ts",
    ...
    fn adaptive_manifest_with_audio_and_subtitles() -> HlsArtifactManifest {
        HlsArtifactManifest::adaptive_fmp4_with_audio(
            "hls",
            "hls/master.m3u8",
            vec![HlsRendition::new(0, 1280, 720, 3_000_000, 128_000)],
    ...
    fn audio_renditions() -> HlsMediaRenditionPlan {
