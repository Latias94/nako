...
use nako_core::{
    ...
    MediaItem, MediaItemId, MediaKind, MediaRepository, MediaSource, MediaSourceId, NakoError,
    ...
    StagingManifestRepository, UserPrincipalId,
};
...
fn storage_apply_kind(
    ...
    match operation_kind {
        ...
        ManagedImportPromotionOperationKind::Move => Err(NakoError::Unsupported(
            "managed import move promotion apply is deferred until source-retention semantics are proven",
        )),
    }
}
...
fn link_plan_status(
    status: nako_vfs::StorageLinkPlanStatus,
) -> ManagedImportPromotionOperationStatus {
    match status {
        nako_vfs::StorageLinkPlanStatus::Ready => ManagedImportPromotionOperationStatus::Ready,
        nako_vfs::StorageLinkPlanStatus::Unsupported => {
            ManagedImportPromotionOperationStatus::Unsupported
        }
        nako_vfs::StorageLinkPlanStatus::SourceMissing => {
            ManagedImportPromotionOperationStatus::SourceMissing
        }
        nako_vfs::StorageLinkPlanStatus::SourceNotFile => {
            ManagedImportPromotionOperationStatus::SourceNotFile
        }
        nako_vfs::StorageLinkPlanStatus::TargetParentMissing => {
            ManagedImportPromotionOperationStatus::TargetParentMissing
        }
        nako_vfs::StorageLinkPlanStatus::TargetParentNotDirectory => {
            ManagedImportPromotionOperationStatus::TargetParentNotDirectory
        }
        nako_vfs::StorageLinkPlanStatus::TargetExists => {
            ManagedImportPromotionOperationStatus::TargetExists
        }
        nako_vfs::StorageLinkPlanStatus::SecurityViolation => {
...
fn safe_storage_planning_message(err: &NakoError) -> String {
    match err {
        NakoError::NotFound { .. } => "storage planning target was not found",
        NakoError::Unsupported(_) => "storage backend does not support promotion planning",
        NakoError::InvalidInput { .. } => "storage planning input is invalid",
        NakoError::Storage { .. } => "storage backend could not plan promotion safely",
