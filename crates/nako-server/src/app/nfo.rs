...
impl NfoAppService {
    ...
    pub(crate) async fn apply_sidecar_apply(
        ...
        match accepted.operation_kind {
            ...
            NfoSidecarApplyOperationKind::RoundTripUpdate => Err(NakoError::Unsupported(
    ...
    async fn execute_nfo_import_command(
        ...
        match self.execute_nfo_import_job(job_id, library_id).await? {
            ...
            NfoImportExecution::Cancelled(job) => Err(NakoError::Conflict {
    ...
    async fn execute_nfo_export_command(
        ...
        match self.execute_nfo_export_job(job_id, library_id).await? {
            ...
            NfoExportExecution::Cancelled(job) => Err(NakoError::Conflict {
...
async fn attempt_export_rollback_from_backup(
    ...
    match backend
        .restore(nako_vfs::StorageRestoreRequest::new(
...
fn storage_restore_status_code(status: nako_vfs::StorageRestoreStatus) -> &'static str {
    match status {
        nako_vfs::StorageRestoreStatus::Restored => "restored",
        nako_vfs::StorageRestoreStatus::Unsupported => "unsupported",
        nako_vfs::StorageRestoreStatus::BackupMissing => "backup_missing",
        nako_vfs::StorageRestoreStatus::BackupNotFile => "backup_not_file",
        nako_vfs::StorageRestoreStatus::TargetParentMissing => "target_parent_missing",
        nako_vfs::StorageRestoreStatus::TargetParentNotDirectory => "target_parent_not_directory",
        nako_vfs::StorageRestoreStatus::SecurityViolation => "security_violation",
        nako_vfs::StorageRestoreStatus::RestoreFailed => "restore_failed",
    }
}

fn nfo_sidecar_audit_commit_failed_error(error: NakoError) -> NakoError {
    NakoError::Conflict {
        message: format!("nfo_sidecar_apply_audit_commit_failed: {error}"),
    }
}

fn nfo_sidecar_audit_and_repair_audit_failed_error() -> NakoError {
    NakoError::Database {
        message:
...
struct NfoAuthorityPreviewSnapshot<'a> {
    ...
    policy: nako_core::LocalMetadataPolicy,
    force: bool,
    scanned_sources: u64,
    create_items: u64,
    skip_items: u64,
...
impl NfoCancellationCheck for DurableNfoCancellationCheck {
    ...
    ) -> nako_core::Result<NfoCancellationDecision> {
