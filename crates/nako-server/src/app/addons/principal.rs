...
use nako_core::{
    ...
    NakoError, NewAddonGrant, Result, hash_addon_token,
};
...
impl AddonAppService {
    pub async fn check_addon_access(
        ...
        authorize_addon_principal_grant(&principal, request.permission, request.library_id)?;
...
pub(super) async fn resolve_addon_principal_from_store(
    store: &NakoDatabase,
    raw_token: &str,
) -> Result<AddonPrincipal> {
    let raw_token = raw_token.trim();
    if raw_token.is_empty() {
        return Err(NakoError::Unauthorized {
    ...
    let token = store
        ...
        .await?
        .ok_or_else(|| NakoError::Unauthorized {
...
pub(super) fn authorize_addon_principal_grant(
