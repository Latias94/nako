...
async fn nfo_export_rejects_read_only_webdav_backend() {
    ...
    assert_eq!(
        err,
        NakoError::Unsupported("NFO export requires a writable storage backend")
    );
}
...
async fn nfo_sidecar_apply_rejects_stale_preview_without_persistence_or_mutation() {
    ...
    assert!(matches!(
        err,
        NakoError::Conflict { message } if message.contains("preview is stale")
    ));
...
async fn nfo_sidecar_apply_exports_forced_update_with_backup_and_retention_diagnostics() {
    ...
    .unwrap();
    fs::write(temp.path().join("demo.nfo.nako-backup-0000"), "old backup").unwrap();
    let library_id = LibraryId::new();
    let config = NakoServerConfig {
        database_backend: Default::default(),
        listen_addr: "127.0.0.1:0".parse().unwrap(),
    ...
    assert!(
        backup_files
            .iter()
            .any(|name| name.starts_with("demo.nfo.nako-backup-"))
    );
...
async fn nfo_sidecar_apply_export_retention_diagnostic_failure_commits_with_redacted_warning() {
    ...
    .unwrap();
    fs::create_dir(temp.path().join("demo.nfo.nako-backup-0000")).unwrap();
    for index in 1..5 {
        fs::write(
            temp.path().join(format!("demo.nfo.nako-backup-000{index}")),
    ...
    assert!(sidecar_xml.contains("<custom>keep</custom>"));
    assert!(temp.path().join("demo.nfo.nako-backup-0000").is_dir());
...
"#,
    ...
    let app = NakoApp::new_with_store(config, store.clone())
        .await
        .unwrap();
    let mut options = LibraryOptions::from_preset(nako_core::LibraryPreset::Tv);
    options.metadata_profile.local_metadata_policy = LocalMetadataPolicy::LocalFirst;
    store
        .upsert_library(&Library {
            id: library_id,
...
impl FailingNfoWriteBackend {
    fn new(root: impl Into<PathBuf>) -> nako_core::Result<Self> {
        Ok(Self {
            inner: LocalFsBackend::new(root)?,
        })
    }

    fn fail_if_nfo_write(&self, uri: &StorageUri) -> nako_core::Result<()> {
...
impl StorageBackend for FailingNfoRestoreBackend {
    ...
    ) -> nako_core::Result<StorageRestoreReport> {
