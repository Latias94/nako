...
use nako_core::{
    ManagedArtworkArtifactId, ManagedArtworkArtifactRecord, NakoError, Result, SelectedArtworkId,
    StorageErrorKind,
};
...
impl LocalManagedArtworkArtifactStore {
    ...
    pub(super) fn path_for_artifact(
        ...
        let extension =
            image_extension_for_media_type(media_type).ok_or_else(|| NakoError::Storage {
                uri: "managed-artwork://artifact".to_owned(),
                kind: StorageErrorKind::Unknown,
...
fn managed_artwork_artifact_store_inventory_error() -> NakoError {
    NakoError::Storage {
        uri: "managed-artwork://artifact".to_owned(),
        kind: StorageErrorKind::Io,
