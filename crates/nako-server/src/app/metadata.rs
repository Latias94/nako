...
impl MetadataAppService {
    ...
    ) -> Result<Vec<nako_metadata::MetadataCandidate>> {
        let lookup = nako_metadata::MetadataLookup {
            kind: Some(item.kind),
            title: item.metadata.title.clone(),
    ...
    async fn create_metadata_maintenance_job(
        ...
        self.validate_metadata_maintenance_request(request).await?;
        let input_json = serde_json::to_string(request).map_err(|err| NakoError::InvalidInput {
    ...
    async fn execute_metadata_maintenance_command(
            ...
            .await?
        {
            ...
            MetadataMaintenanceExecution::Cancelled(job) => Err(NakoError::Conflict {
...
fn validate_profile_applies_to_item_kind(
    ...
    if !profile.item_kinds.is_empty()
        ...
        && !profile.item_kinds.contains(&nako_core::MediaKind::Unknown)
    {
...
fn metadata_raw_retention_cutoff(retention_ms: u64) -> Result<String> {
    let millis = i64::try_from(retention_ms).map_err(|_| NakoError::InvalidInput {
