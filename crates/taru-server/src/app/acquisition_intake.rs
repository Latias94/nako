...
use taru_core::{
    ...
    ManagedImportSourceKind, NewAcquisitionIntakeCandidate, PageRequest, Result, TaruError,
};
use taru_db::TaruDatabase;
use taru_library::LibraryScannerOptions;
...
impl WatchFolderDiscoveryFailureDiagnostic {
    fn from_error(uri: StorageUri, err: TaruError) -> Self {
        Self {
...
fn safe_error_message(err: &TaruError) -> String {
    match err {
        TaruError::NotFound { entity, .. } => format!("{entity} was not found"),
        TaruError::InvalidInput { .. } => "invalid watch-folder input".to_owned(),
        TaruError::Conflict { .. } => "watch-folder conflict".to_owned(),
        TaruError::Unauthorized { .. } => "watch-folder access is unauthorized".to_owned(),
        TaruError::Forbidden { .. } => "watch-folder access is forbidden".to_owned(),
        TaruError::Unsupported(_) => "watch-folder operation is unsupported".to_owned(),
        TaruError::Provider { provider, .. } => format!("{provider} provider error"),
        TaruError::Storage { kind, .. } => format!("storage error: {kind:?}"),
        TaruError::Database { .. } => "database error".to_owned(),
    }
}
