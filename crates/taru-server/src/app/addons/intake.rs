use taru_addon_protocol::AddonScope;
...
use taru_core::{
    ...
    LibraryItemRepository, LibraryRepository, MediaRepository, NewAutomationArtifact,
    NewAutomationProviderConfig, NewJob, Result, TaruError,
};
...
impl AddonAppService {
    ...
    async fn validate_generated_artifact_target(
        ...
        let item_id = request.item_id.ok_or_else(|| TaruError::InvalidInput {
    ...
    async fn find_generated_artifact_replay(
        ...
        request: &AddonGeneratedArtifactRuntimeRequest,
    ) -> Result<Option<taru_core::AutomationArtifactRecord>> {
        let item_id = request.item_id.ok_or_else(|| TaruError::InvalidInput {
            message: "addon generated artifact requires a media item target".to_owned(),
        })?;
        for artifact in self
            ...
            .list_automation_artifacts_for_item(
                item_id,
                taru_core::PageRequest::new(taru_core::PageRequest::MAX_LIMIT, 0),
            )
            .await?
        {
    ...
    async fn addon_automation_provider_capabilities(
        &self,
        provider_id: AutomationProviderId,
        capability: taru_core::AutomationCapability,
    ) -> Result<Vec<taru_core::AutomationCapability>> {
...
struct AddonGeneratedArtifactRuntimeRequest {
    capability: taru_core::AutomationCapability,
    kind: taru_core::AutomationArtifactKind,
    library_id: Option<LibraryId>,
    item_id: Option<taru_core::MediaItemId>,
    source_id: Option<taru_core::MediaSourceId>,
...
fn serialize_json_field(label: &str, value: &serde_json::Value) -> Result<String> {
    serde_json::to_string(value).map_err(|err| TaruError::InvalidInput {
...
fn addon_automation_provider_id(manifest_id: &str) -> AutomationProviderId {
    ...
    let digest = Sha256::digest(format!("taru.addon.automation-provider.v1:{manifest_id}"));
