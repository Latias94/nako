...
use taru_core::{MediaItemId, MediaSourceId, TaruError, UserPlaybackState, UserPrincipalId};
...
fn parse_optional_media_source_id(
    value: Option<String>,
) -> Result<Option<MediaSourceId>, TaruError> {
    value
        .map(|value| {
            value
                .parse::<MediaSourceId>()
...
fn parse_optional_rfc3339_ms(value: Option<&str>) -> Result<Option<i64>, TaruError> {
    value.map(parse_rfc3339_ms).transpose()
}

fn parse_rfc3339_ms(value: &str) -> Result<i64, TaruError> {
    let timestamp =
        OffsetDateTime::parse(value, &Rfc3339).map_err(|err| TaruError::InvalidInput {
    ...
    i64::try_from(millis).map_err(|err| TaruError::InvalidInput {
