...
use taru_core::{
    ...
    AutomationProviderStatus, AutomationRepository, Job, JobId, JobKind, JobRepository,
    NewAutomationArtifact, NewJob, Result, TaruError,
};
use tokio::time;
    ...
    P: AutomationProvider,
{
    ...
    async fn run_provider(
        ...
        let prompt =
            serde_json::from_str(&input.prompt_json).map_err(|err| TaruError::InvalidInput {
        ...
        for attempt in 1..=provider.max_attempts {
            ...
            let result = tokio::select! {
                ...
                result = time::timeout(timeout, provider_run) => {
                    result.map_err(|_| TaruError::Provider {
                        provider: provider.name.clone(),
        ...
        let outcome = outcome.ok_or_else(|| {
            last_error.unwrap_or_else(|| TaruError::Provider {
                provider: provider.name.clone(),
        ...
        let output_json =
            serde_json::to_string(&outcome.output).map_err(|err| TaruError::InvalidInput {
...
fn parse_job_input(job: &Job) -> Result<AutomationJobInput> {
    ...
    serde_json::from_str(input_json).map_err(|err| TaruError::InvalidInput {
...
fn safe_error(error: &TaruError) -> String {
    match error {
        TaruError::Provider { provider, message } => {
            format!("automation provider {provider} failed: {message}")
        }
        TaruError::Storage { .. } => "automation job storage operation failed".to_owned(),
        TaruError::Database { .. } => "automation job database operation failed".to_owned(),
