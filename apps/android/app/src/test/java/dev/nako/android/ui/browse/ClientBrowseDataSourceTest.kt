...
class ClientBrowseDataSourceTest {
    ...
    fun `detail version probe and playback decision fail safely without saved access`() = runBlocking {
        ...
        assertEquals(emptyList<NakoHttpRequest>(), transport.requests)
    }
...
private fun testDataSource(transport: NakoHttpTransport): ClientBrowseDataSource {
    val vault = InMemoryTokenVault()
    val profile = testProfile()
    vault.saveToken(profile.tokenReference, "secret-token")
...
private class RecordingTransport : NakoHttpTransport {
    val requests: MutableList<NakoHttpRequest> = mutableListOf()

    override suspend fun execute(request: NakoHttpRequest): NakoHttpResponse {
        requests += request
        return NakoHttpResponse(statusCode = 500)
    }
}

private class QueuedTransport(
    vararg responses: NakoHttpResponse,
) : NakoHttpTransport {
