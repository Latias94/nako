...
import dev.nako.android.browse.GenreDto
...
import dev.nako.android.browse.SearchItemHit
import dev.nako.android.browse.SearchResponse
import dev.nako.android.browse.TagDto
...
class BrowseSessionLoadingTest {
    ...
    fun `relationship index load more appends rows with server page semantics`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.RelationshipIndex(RelationshipIndexFamily.Genres)))?.join()
    ...
    fun `facet load more appends related items and preserves current route`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.BrowseFacet(target)))?.join()
        session.dispatch(BrowseAction.LoadMoreFacet)?.join()

        val content = session.state.value.facetState as FacetUiState.Content
        assertEquals(NakoRoute.BrowseFacet(target), session.state.value.currentRoute)
    ...
    fun `item detail load selects first source and starts source probe`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.ItemDetail("night-harbor")))?.join()
    ...
    fun `start playback opens player route with prepared target`() = runBlocking {
        ...
        val playerRoute = session.state.value.currentRoute as NakoRoute.Player
    ...
    fun `start playback failure keeps diagnostics in playback state`() = runBlocking {
        ...
        assertEquals(NakoRoute.ItemDetail("night-harbor"), session.state.value.currentRoute)
        assertEquals(
            diagnostics,
    ...
    fun `library detail route load ignores stale response after back`() = runBlocking {
        ...
        session.dispatch(BrowseAction.SelectDestination(NakoDestination.Libraries))
        session.dispatch(BrowseAction.OpenLibraryDetail("library-movies"))
        val job = session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.LibraryDetail("library-movies")))
        session.dispatch(BrowseAction.Back)

        job?.join()
        assertEquals(NakoRoute.TopLevel, session.state.value.currentRoute)
    ...
    fun `person detail route loads person and related media items`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.PersonDetail("person-1")))?.join()
    ...
    fun `person detail route load ignores stale response after back`() = runBlocking {
        ...
        session.dispatch(BrowseAction.OpenPersonDetail("person-1"))
        val job = session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.PersonDetail("person-1")))
        session.dispatch(BrowseAction.Back)
    ...
    fun `relationship index route loads genre rows and retry reloads current index`() = runBlocking {
        ...
        assertEquals(NakoRoute.BrowseFacet(row.target), session.state.value.currentRoute)
    }
    ...
    fun `relationship index route loads tag rows into tag facet route`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.RelationshipIndex(RelationshipIndexFamily.Tags)))?.join()
    ...
    fun `relationship index route load ignores stale response after back`() = runBlocking {
        ...
        session.dispatch(BrowseAction.OpenRelationshipIndex(RelationshipIndexFamily.Genres))
        val job = session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.RelationshipIndex(RelationshipIndexFamily.Genres)))
        session.dispatch(BrowseAction.Back)
    ...
    fun `public backed facet loads content and unsupported facet stays local api gap`() = runBlocking {
        ...
        session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.BrowseFacet(backedFacet)))?.join()
        ...
        val gapJob = session.dispatch(BrowseAction.RouteDisplayed(NakoRoute.BrowseFacet(unsupportedFacet)))
...
private fun testItems(id: String): dev.nako.android.browse.ItemsResponse =
    dev.nako.android.browse.ItemsResponse(
