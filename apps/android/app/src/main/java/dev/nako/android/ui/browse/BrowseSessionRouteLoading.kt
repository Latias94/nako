...
internal class BrowseRouteLoadingSession(
    ...
    fun loadRoute(route: NakoRoute): Job? =
        when (route) {
            is NakoRoute.ItemDetail -> detailSession.loadItemDetail(route.itemId)
            is NakoRoute.LibraryDetail -> loadLibraryDetail(route.libraryId)
            is NakoRoute.PersonDetail -> loadPersonDetail(route.personId)
            is NakoRoute.RelationshipIndex -> loadRelationshipIndex(route.family)
            is NakoRoute.BrowseFacet -> loadFacet(route.target)
    ...
    fun loadMoreRelationshipIndex(): Job? {
        val route = store.value.currentRoute as? NakoRoute.RelationshipIndex ?: return null
        val currentContent =
    ...
    fun loadMoreFacet(): Job? {
        val route = store.value.currentRoute as? NakoRoute.BrowseFacet ?: return null
    ...
    private fun loadLibraryDetail(libraryId: String): Job {
        ...
        return scope.launch {
            ...
            store.update { current ->
                val routeStillCurrent = current.currentRoute == NakoRoute.LibraryDetail(libraryId)
    ...
    private fun loadPersonDetail(personId: String): Job {
        ...
        return scope.launch {
            ...
            store.update { current ->
                val routeStillCurrent = current.currentRoute == NakoRoute.PersonDetail(personId)
    ...
    private fun loadRelationshipIndex(family: RelationshipIndexFamily): Job {
        ...
        return scope.launch {
            ...
            store.update { current ->
                val routeStillCurrent = current.currentRoute == NakoRoute.RelationshipIndex(family)
                if (routeStatePolicy.acceptsRelationshipIndex(requestId) && routeStillCurrent) {
    ...
    private fun loadFacet(target: BrowseFacetTarget): Job? {
        ...
        return scope.launch {
            ...
            store.update { current ->
                val routeStillCurrent = current.currentRoute == NakoRoute.BrowseFacet(target)
                if (routeStatePolicy.acceptsFacet(requestId) && routeStillCurrent) {
